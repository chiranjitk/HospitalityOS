...
> **Verification date: 2026-05-28 (round 2)** — All 8 items re-verified against the ENTIRE codebase (not just PMS pages). Item #13 was a false positive. Items #14–#20 confirmed as genuine gaps, with additional context from the billing/frontdesk modules.
>
> **Key discovery:** The initial analysis only scanned the 13 PMS pages under the Property Management menu. Many of the "missing" capabilities exist in OTHER modules (Channel Manager, Revenue, Billing, Frontdesk). The P2 items below are gaps where the feature is genuinely missing even when considering ALL modules.

| # | Feature | Status |
|---|---------|--------|
| 13 | ~~Channel-specific availability/rate allocation~~ | **ALREADY FULLY IMPLEMENTED** — Complete Channel Manager module: 32 UI components, 36+ API routes. See `src/components/channels/` and `src/app/api/channels/`. |
| 14 | Rate plan categories + versioning + approval | **Genuine Gap** — No `category` field on RatePlan model (BAR/Corporate/OTA/Package exist only as plan names, not categories). No `RatePlanVersion` model or history tracking. No approval workflow (status is binary active/inactive only). Note: Other modules have approval workflows (AP invoices, room type changes, BEOs, purchase requisitions) that could serve as architectural references. |
| 15 | Child age policies + occupancy tiers | **Genuine Gap (PMS booking engine)** — RoomType model has no age bracket thresholds (infant/child/adult). PMS booking engine accepts raw counts and validates against `maxOccupancy`/`maxAdults` only — no age classification or per-guest-type pricing. Note: Channel Manager has full child age config (`ChannelGuestRateConfig` model + `guest-rates.tsx` UI with infant/child/adult thresholds, extra adult/child rates, crib/extra bed/rollaway). Revenue module has property-level occupancy-based pricing tiers (5 tiers in `hourly-pricing-engine.ts`) but these use property occupancy %, not guest composition. |
| 16 | Package edit UI + type classification | **Genuine Gap** — Backend `PUT /api/packages/[id]` and `PUT /api/packages/rates/[id]` exist but NO frontend edit dialog (dropdown has only View Details/Add Rate/Delete). No `packageType`/`packageCategory` field in schema (seed data uses suggestive names like "Honeymoon Bliss" but no structured classification). |
| 17 | Real-time floor plan status (WebSocket) | **Genuine Gap** — Full WebSocket infrastructure exists (`useRealtime`, `useRoomStatus` hooks, `SocketProvider`, realtime-service on port 3003) and IS used by `frontdesk/room-grid.tsx` for live room status. But all 4 floor plan components (`floor-plans.tsx`, `floor-plan-editor.tsx`, `floor-plan-viewer.tsx`, `room-floor-plan-widget.tsx`) only do static `fetch()` on mount — zero real-time subscription. The `useRoomStatus()` hook is purpose-built and ready to wire in. |
| 18 | Room type change folio charge creation | **Genuine Gap (wiring issue, not missing module)** — Full folio system exists in Billing module: `components/billing/folios.tsx`, `POST /api/folios/[id]/line-items` for charge creation, `FolioLineItem` model with 17+ category types. Two room type change systems exist: (1) `/api/pms/room-type-change/[id]` — buggy: `folio.updateMany` references non-existent `roomId`/`notes` fields on Folio model, crashes silently in try/catch, never creates FolioLineItem; (2) `/api/rooms/type-changes/[id]/approve` — creates FolioLineItem correctly but uses `category: 'room_type_change'` which is NOT in the `FolioLineType` enum. Fix: add `room_type_change` to enum + rewrite completion handler to use `folioLineItem.create` pattern from billing module. |
| 19 | Drag-to-set rates on calendars | **Genuine Gap** — No drag-to-select exists in ANY rate calendar. `room-rate-calendar.tsx` and `inventory-calendar.tsx` support only click-to-edit single cells + bulk dialog. Note: `booking-calendar.tsx` HAS drag for booking date-range creation/movement, and `event-calendar.tsx` has drag for event rescheduling — so the drag pattern exists in the codebase but has never been applied to rate calendars. |
| 20 | Adjoining/connecting rooms | **Genuine Gap** — No `RoomConnection` model, no self-referencing Room relation, no UI for linking rooms anywhere. Guest profile has a dead `connectingRoom: boolean` preference toggle (`guest-preferences.tsx`, `preferences-management.tsx`) but nothing reads or acts on it — not the smart room assigner, not the auto-assign API. Floor plan editor stores x/y coordinates but computes no adjacency. |
