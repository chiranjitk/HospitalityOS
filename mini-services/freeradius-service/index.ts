...
// This fixes the connection errors issue where FreeRADIUS can't INSERT because
// the table is missing columns or doesn't exist.
// =========================================================================

// Run radacct table check on startup

// Initialize database tables asynchronously (wrapped in IIFE to avoid top-level await issues with PM2)
(async () => {
  try {
    // RadiusEvent table — ensure it exists in PostgreSQL
    await pool.query(`
    ...
    log.info('RadiusEvent table verified in PostgreSQL');
  } catch (err) {
    log.error('Failed to verify RadiusEvent table in PostgreSQL', { error: String(err) });
  }
})();
...
async function getNASClientById(id: string | number): NASClient | null {
  const row = await db.query('SELECT * FROM nas WHERE id = ?').get(Number(id)) as Record<string, unknown> | null;
  return row ? rowToNASClient(row) : null;
}

async function createNASClient(client: NASClient): number {
...
async function updateNASClient(id: string | number, updates: Partial<NASClient> & { coaEnabled?: boolean }): NASClient | null {
  const existing = await getNASClientById(id);
...
async function deleteNASClient(id: string | number): boolean {
  const result = await db.query('DELETE FROM nas WHERE id = ?').run(Number(id));
  return result.rowCount > 0;
}

// ---- FreeRADIUS Native SQL User Functions ----

async function getRADIUSUserByUsername(username: string): RADIUSUser | null {
  ...
  const checkRows = await db.query('SELECT * FROM radcheck WHERE username = ?').all(username) as Array<Record<string, unknown>>;
...
async function createRADIUSUser(user: RADIUSUser): void {
  ...
  await db.query('DELETE FROM radcheck WHERE username = ? AND attribute = ?').run(user.username, 'Cleartext-Password');
  await insertRadCheck(user.username, 'Cleartext-Password', ':=', user.password);

  // Insert attributes into radreply (via prepared statement)
  await db.query('DELETE FROM radreply WHERE username = ?').run(user.username);
  for (const [attr, val] of Object.entries(user.attributes)) {
    await insertRadReply(user.username, attr, '=', val);
  }

  // Insert group mapping
  if (user.group) {
    await db.query('DELETE FROM radusergroup WHERE username = ?').run(user.username);
    await insertRadUserGroup(user.username, user.group, 0);
  }
}

async function updateRADIUSUser(id: string, updates: Partial<RADIUSUser>): RADIUSUser | null {
  const existing = await getRADIUSUserById(id);
  if (!existing) return null;
  ...
  await db.query('DELETE FROM radcheck WHERE username = ? AND attribute = ?').run(username, 'Cleartext-Password');
  await insertRadCheck(username, 'Cleartext-Password', ':=', updated.password);

  // Insert attributes into radreply
  await db.query('DELETE FROM radreply WHERE username = ?').run(username);
  for (const [attr, val] of Object.entries(updated.attributes || {})) {
    await insertRadReply(username, attr, '=', val);
  }

  // Insert group mapping
  if (updated.group) {
    await db.query('DELETE FROM radusergroup WHERE username = ?').run(username);
    await insertRadUserGroup(username, updated.group, 0);
  }

  return updated;
}

async function deleteRADIUSUser(id: string): boolean {
  const existing = await getRADIUSUserById(id);
...
async function getRADIUSGroupByName(name: string): RADIUSGroup | null {
  const checkRows = await db.query('SELECT * FROM radgroupcheck WHERE groupname = ?').all(name) as Array<Record<string, unknown>>;
  const replyRows = await db.query('SELECT * FROM radgroupreply WHERE groupname = ?').all(name) as Array<Record<string, unknown>>;
...
async function getAllRADIUSGroups(): RADIUSGroup[] {
  const rows = await db.query('SELECT DISTINCT groupname FROM radgroupcheck UNION SELECT DISTINCT groupname FROM radgroupreply')
    .all() as Array<{ groupname: string }>;
  const groups = await Promise.all(rows.map(async r => await getRADIUSGroupByName(r.groupname)));
  return groups.filter((g): g is RADIUSGroup => g !== null);
}

async function createRADIUSGroup(group: RADIUSGroup): void {
  // Delete existing if present (idempotent)
  await db.query('DELETE FROM radgroupcheck WHERE groupname = ?').run(group.name);
  await db.query('DELETE FROM radgroupreply WHERE groupname = ?').run(group.name);

  for (const [attr, val] of Object.entries(group.checkAttributes)) {
    await insertRadGroupCheck(group.name, attr, ':=', val);
  }
  for (const [attr, val] of Object.entries(group.replyAttributes)) {
    await insertRadGroupReply(group.name, attr, '=', val);
  }
}

async function deleteRADIUSGroup(name: string): boolean {
  const existing = await getRADIUSGroupByName(name);
  if (!existing) return false;

  await db.query('DELETE FROM radgroupcheck WHERE groupname = ?').run(name);
  await db.query('DELETE FROM radgroupreply WHERE groupname = ?').run(name);
 ...
 */
async function resolveEventGroupFromPlan(eventId: string, eventName: string): string {
  try {
 ...
 */
async function lookupAllNASVendors(): string[] {
  try {
    const rows = await db.query("SELECT DISTINCT type FROM nas WHERE status = 'active'").all() as { type: string }[];
 ...
 * NOTE: Prefer await lookupAllNASVendors() for multi-NAS environments.
 */
async function lookupNASVendor(): string {
  const vendors = await lookupAllNASVendors();
 ...
 */
async function lookupNASVendorByIP(nasIp: string): string {
  try {
    const row = await db.query("SELECT type FROM nas WHERE nasname = ? AND status = 'active'").get(nasIp) as { type: string } | null;
 ...
 */
async function generateVendorAttributes(params: {
  downloadBps: number;
  uploadBps: number;
  sessionTimeoutMinutes: number;
  dataLimitMB: number;
  ...
  if (params.nasIp) {
    ...
    const nasVendor = await lookupNASVendorByIP(params.nasIp);
    ...
    const allNasVendors = await lookupAllNASVendors();
...
// PostgreSQL diagnostic — verify connection and table status
...
app.get('/api/status', async (c) => {
  ...
  const nasClients = await getAllNASClients();
  const radiusUsers = await getAllRADIUSUsers();
  const radiusGroups = await getAllRADIUSGroups();
  return c.json({
    success: true,
    data: {
      ...status,
...
app.post('/api/nas', async (c) => {
  try {
    ...
    const newId = await createNASClient(client);
...
app.post('/api/nas/generate-secret', async (c) => {
  return c.json({
    success: true,
    data: {
...
app.get('/api/users/:id', async (c) => {
  const { id } = c.req.param();
  const user = await getRADIUSUserById(id);
...
app.get('/api/groups', async (c) => {
  ...
  const customGroups = await getAllRADIUSGroups();
...
app.post('/api/config/import', async (c) => {
  try {
    ...
    if (body.nas) {
      ...
      await db.query('DELETE FROM nas').run();
      for (const n of body.nas as NASClient[]) {
        const client: NASClient = {
          ...n,
          updatedAt: new Date().toISOString()
        };
        await createNASClient(client);
      }
    }

    if (body.users) {
      // Clear existing user data from all related tables
      await db.query('DELETE FROM radcheck').run();
      await db.query('DELETE FROM radreply').run();
      await db.query('DELETE FROM radusergroup').run();
      // Only delete from tables that exist
      try { await db.query('DELETE FROM user_metadata').run(); } catch { /* table may not exist */ }
    ...
    if (body.groups) {
      ...
      await db.query('DELETE FROM radgroupcheck').run();
      await db.query('DELETE FROM radgroupreply').run();
...
app.get('/api/config/default', async (c) => {
  return c.json({
    success: true,
    data: {
      authentication: {
...
async function syncAllConfigFiles(): Promise<void> {
  log.info('Starting RADIUS config sync from PostgreSQL database');
...
app.post('/api/provision', async (c) => {
  try {
    ...
    const existing = await getRADIUSUserByUsername(username);
    if (existing) {
...
// ============================================================================
// FreeRADIUS SQL Auto-Setup — configure radiusd to read from PostgreSQL
// ============================================================================
 ...
 */
async function verifyRadiusTables(): void {
  const tables = [
    'radcheck',
    'radreply',
    'radgroupcheck',
  ...
  for (const tableName of tables) {
    try {
      const src = await db.query("SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND table_name=?").get(tableName);
      if (src) {
...
/**
 * Configure FreeRADIUS to use the PostgreSQL database via rlm_sql.
 *
 * Steps:
...
async function setupFreeRadiusSQL(): Promise<{ success: boolean; error?: string; details: string[] }> {
  ...
  try {
    ...
    // Step 1c: PostgreSQL handles WAL and concurrency natively
    details.push('PostgreSQL manages its own WAL and concurrency settings');
...
`;
    ...
    try {
      ...
      if (acctSection && !acctSection[1].includes('sql')) {
        sitesContent = sitesContent.replace(
          /accounting\s*\{/,
          'accounting {\n\t# StaySuite: Log accounting to PostgreSQL\n\tsql'
        );
    ...
    // Step 6: PostgreSQL handles its own file permissions
    // No need to chown/chmod database files like with SQLite
    details.push('PostgreSQL manages its own file permissions');
    ...
    sqlSetupDone = true;
    sqlSetupStatus = { enabled: true, database: DATABASE_URL };

    return { success: true, details };
  } catch (error) {
    const errMsg = String(error);
    sqlSetupStatus = { enabled: false, database: DATABASE_URL, error: errMsg };
...
app.get('/api/config/sql-status', async (c) => {
 ...
 */
async function getAccountingSessionsFromDB(): AccountingSession[] {
  try {
...
app.get('/api/accounting', async (c) => {
  try {
    ...
    if (usedFallback) {
      allSessions = await getAccountingSessionsFromDB();
    }
 ...
 */
async function lookupNAS(nasIp?: string): { ip: string; secret: string; coaPort: number } | null {
  const clients = await getAllNASClients();
  if (nasIp) {
 ...
 */
async function logCoaAction(params: {
...
app.post('/api/coa/disconnect', async (c) => {
  try {
    ...
    const nas = await lookupNAS(bodyNasIp);
    if (!nas) {
    ...
    if (radResult.success) {
      try {
        ...
        for (const attr of bwAttrs) {
          await db.query("DELETE FROM radreply WHERE username = ? AND attribute = ?").run(username, attr);
        }
        // Write vendor-appropriate attributes (same as generateBandwidthAttributes)
        const allVendors = await lookupAllNASVendors().map(v => normalizeVendor(v));
...
app.post('/api/coa/disconnect-all', async (c) => {
  try {
    ...
    for (const session of sessions) {
      const nas = await lookupNAS(session.nasipaddress);
      if (!nas) {
        failCount++;
...
app.get('/api/data-cap/check', async (c) => {
  try {
    const username = c.req.query('username') || '';
    if (!username) {
    ...
    const capAttrRow = await db.query(
    ...
    if (capAttrRow) {
      ...
      if (capAttrRow.attribute === 'ChilliSpot-Max-Input-Octets' || capAttrRow.attribute === 'ChilliSpot-Max-Output-Octets') {
        ...
        const otherRow = await db.query("SELECT value FROM radreply WHERE username = ? AND attribute = ?").get(username, otherAttr) as { value: string } | undefined;
...
app.post('/api/mac-auth', async (c) => {
  try {
    ...
    if (autoLogin) {
      try {
        await db.query("DELETE FROM radcheck WHERE username = ? AND attribute = 'Calling-Station-Id'").run(macForRadius);
        await insertRadCheck(macForRadius, 'Calling-Station-Id', ':=', macForRadius);
        // Set a known password for the MAC user
        await db.query("DELETE FROM radcheck WHERE username = ? AND attribute = 'Cleartext-Password'").run(macForRadius);
...
app.put('/api/mac-auth/:id', async (c) => {
  try {
    ...
    const existing = await db.query('SELECT * FROM "RadiusMacAuth" WHERE id = ?').get(id) as Record<string, unknown> | null;
    if (!existing) {
...
app.delete('/api/mac-auth/:id', async (c) => {
  try {
    ...
    await db.query("DELETE FROM radcheck WHERE username = ? AND attribute = 'Calling-Station-Id'").run(mac);
    await db.query("DELETE FROM radcheck WHERE username = ? AND attribute = 'Cleartext-Password'").run(mac);

    await db.query('DELETE FROM "RadiusMacAuth" WHERE id = ?').run(id);
...
app.post('/api/mac-auth/import', async (c) => {
  try {
    ...
    for (const item of macs) {
      ...
      const existing = await db.query('SELECT id FROM "RadiusMacAuth" WHERE macAddress = ?').get(mac);
      ...
      db.query(
        `INSERT INTO "RadiusMacAuth" (id, propertyId, macAddress, username, description, autoLogin, validFrom, validUntil, status, createdAt, updatedAt)
         VALUES (?, 'property-1', ?, ?, ?, 1, NOW(), NULL, 'active', NOW(), NOW())`
...
app.get('/api/event-users', async (c) => {
  try {
    ...
    try {
      explicitEvents = await db.query('SELECT * FROM "RadiusEvent" ORDER BY createdAt DESC').all() as Record<string, unknown>[];
...
app.post('/api/event-users/bulk', async (c) => {
  try {
    ...
    for (let i = 1; i <= bulkCount; i++) {
      ...
      const groupName = await resolveEventGroupFromPlan(eventId, eventName);
...
app.post('/api/event-users/:id/revoke', async (c) => {
  try {
    ...
    const existing = await db.query('SELECT * FROM "RadiusEventUser" WHERE id = ?').get(id) as Record<string, unknown> | null;
    if (!existing) {
...
app.delete('/api/event-users/bulk', async (c) => {
  try {
    ...
    for (const uid of eventUserIds) {
      const existing = await db.query('SELECT username FROM "RadiusEventUser" WHERE id = ?').get(uid) as { username: string } | null;
      if (existing) {
...
app.post('/api/event-users/attendee', async (c) => {
  try {
    ...
    const event = await db.query('SELECT * FROM "RadiusEvent" WHERE id = ?').get(eventId) as Record<string, unknown> | null;
    ...
    // Resolve RADIUS group from WiFi plan linked to the event (not the event name)
    const groupName = await resolveEventGroupFromPlan(eventId, effectiveEventName);
    const user: RADIUSUser = {
      id, username, password,
      group: groupName,
      attributes: attrs,
...
app.delete('/api/event-users/event/:eventId', async (c) => {
  try {
    ...
    const eventUsers = await db.query('SELECT username, id FROM "RadiusEventUser" WHERE eventId = ?').all(eventId) as { username: string; id: string }[];
    ...
    await db.query('DELETE FROM "RadiusEventUser" WHERE eventId = ?').run(eventId);

    // 4. Delete the event itself
    await db.query('DELETE FROM "RadiusEvent" WHERE id = ?').run(eventId);
...
app.get('/api/portal-whitelist', async (c) => {
  try {
    const propertyId = c.req.query('propertyId') || '';
...
app.put('/api/portal-whitelist/:id', async (c) => {
  try {
    ...
    const existing = await db.query('SELECT * FROM "PortalWhitelist" WHERE id = ?').get(id) as Record<string, unknown> | null;
    if (!existing) {
...
app.delete('/api/portal-whitelist/:id', async (c) => {
  try {
    const { id } = c.req.param();
    const result = await db.query('DELETE FROM "PortalWhitelist" WHERE id = ?').run(id);
  ...
  try {
    ...
    for (const wu of wifiUsers) {
      try {
        ...
        const existing = await db.query("SELECT id FROM radcheck WHERE username = ? AND attribute = 'Cleartext-Password'").get(username);
        if (!existing) {
          await db.query("DELETE FROM radcheck WHERE username = ? AND attribute = 'Cleartext-Password'").run(username);
          await insertRadCheck(username, 'Cleartext-Password', ':=', password);
        }
        ...
        if (sessionTimeout) {
          await db.query("DELETE FROM radreply WHERE username = ? AND attribute = 'Session-Timeout'").run(username);
          await insertRadReply(username, 'Session-Timeout', '=', String(parseInt(String(sessionTimeout), 10) * 60));
        }
 ...
 */
async function setSimultaneousUse(groupName: string, maxSessions: number): void {
  if (maxSessions <= 0) {
    // Remove the limit (unlimited)
    await db.query("DELETE FROM radgroupcheck WHERE groupname = ? AND attribute = 'Simultaneous-Use'").run(groupName);
 ...
 */
async function setAllSimultaneousUse(limits: Record<string, number>): void {
...
app.get('/api/accounting/db', async (c) => {
  try {
    const limit = Math.min(parseInt(c.req.query('limit') || '100', 10) || 100, 1000);
    const offset = parseInt(c.req.query('offset') || '0', 10) || 0;
    const username = c.req.query('username') || '';
    ...
    // Summary stats
    const activeRow = await db.query("SELECT COUNT(*) as c FROM radacct WHERE acctstoptime IS NULL").get() as { c: number } | undefined;
 ...
 */
app.get('/api/user-usage/summary', async (c) => {
  try {
    ...
    const summarySql = `
      ...
      LIMIT ?
    `;
    const summaryRows = await db.query(summarySql).all(...whereParams, limit) as Array<Record<string, unknown>>;
    ...
    const topUserRow = await db.query(
 ...
 */
app.get('/api/user-usage/:username', async (c) => {
  try {
      ...
      for (const u of justExpired) {
        try {
          ...
          for (const s of activeSessions) {
            const nasInfo = await lookupNAS(s.nasipaddress);
            if (nasInfo) {
  ...
  try {
    ...
    for (const u of cappedUsers) {
      ...
      if (u.attribute === 'ChilliSpot-Max-Input-Octets' || u.attribute === 'ChilliSpot-Max-Output-Octets') {
        ...
        const otherRow = await db.query("SELECT value FROM radreply WHERE username = ? AND attribute = ?").get(u.username, otherAttr) as { value: string } | undefined;
...
app.get('/api/concurrent-sessions', async (c) => {
  try {
    ...
    // Also get all groups
    const allGroups = await db.query('SELECT DISTINCT groupname FROM radgroupcheck').all() as Array<{ groupname: string }>;
...
app.get('/api/concurrent-sessions/violations', async (c) => {
  try {
    // Find users with active sessions exceeding their Simultaneous-Use limit
    const groups = await db.query("SELECT groupname, value FROM radgroupcheck WHERE attribute = 'Simultaneous-Use'").all() as Array<{ groupname: string; value: string }>;
...
app.get('/api/provisioning-logs', async (c) => {
  try {
...
app.get('/api/content-filter', async (c) => {
  try {
    const propertyId = c.req.query('propertyId') || '';
...
app.post('/api/content-filter', async (c) => {
  try {
    ...
    const now = new Date().toISOString();
    const { tenantId: resolvedTenantId, propertyId: resolvedPropertyId } = await resolveTenantAndProperty(tenantId, propertyId);
    db.query(
...
app.put('/api/content-filter/:id', async (c) => {
  try {
    ...
    const existing = await db.query('SELECT * FROM "ContentFilter" WHERE id = ?').get(id) as Record<string, unknown> | undefined;
...
app.delete('/api/content-filter/:id', async (c) => {
  try {
    const id = c.req.param('id');
    await db.query('DELETE FROM "ContentFilter" WHERE id = ?').run(id);
...
app.post('/api/content-filter/preset', async (c) => {
  try {
    ...
    for (const rule of rules) {
      ...
      db.query(
        `INSERT INTO "ContentFilter" (id, tenantId, propertyId, name, category, domains, enabled, createdAt, updatedAt)
         VALUES (?, 'tenant-1', 'property-1', ?, ?, ?, 1, NOW(), NOW())`
...
app.get('/api/content-filter/test', async (c) => {
  try {
...
app.get('/api/guest-wifi-link', async (c) => {
  try {
  ...
  try {
    ...
    if (wifiUser) {
      ...
      try {
        const nas = await lookupNAS();
        if (nas) {
          const sessions = await db.query("SELECT acctsessionid FROM radacct WHERE username = ? AND acctstoptime IS NULL").all(username) as Array<{ acctsessionid: string }>;
          for (const session of sessions) {
            const attrs = `User-Name="${username}"\nAcct-Session-Id="${session.acctsessionid}"`;
...
app.get('/api/bandwidth-schedules', async (c) => {
  try {
    const propertyId = c.req.query('propertyId') || '';
    const enabled = c.req.query('enabled') || '';
...
app.post('/api/bandwidth-schedules', async (c) => {
  try {
    ...
    const created = await db.query('SELECT * FROM "ScheduleAccess" WHERE id = ?').get(id) as Record<string, unknown>;
    return c.json({
      success: true,
...
app.put('/api/bandwidth-schedules/:id', async (c) => {
  try {
    ...
    const existing = await db.query('SELECT * FROM "ScheduleAccess" WHERE id = ?').get(id) as Record<string, unknown> | undefined;
    ...
    params.push(id);
    await db.query(`UPDATE "ScheduleAccess" SET ${updates.join(', ')} WHERE id = ?`).run(...params);

    const updated = await db.query('SELECT * FROM "ScheduleAccess" WHERE id = ?').get(id) as Record<string, unknown>;
    return c.json({
      success: true,
...
app.delete('/api/bandwidth-schedules/:id', async (c) => {
  try {
    const id = c.req.param('id');
    await db.query('DELETE FROM "ScheduleAccess" WHERE id = ?').run(id);
...
async function applyBandwidthCoA(
  ...
  const nas = await lookupNAS(nasIpAddress);
  if (!nas) {
...
app.get('/api/live-sessions/stats', async (c) => {
  try {
    const propertyId = c.req.query('propertyId') || '';
    ...
    const totalActive = (await db.query(`SELECT COUNT(*) as c FROM "LiveSession"${where}${andWhere} status = 'active'`).get(...params) as { c: number } | undefined)?.c || 0;
    const totalStale = (await db.query(`SELECT COUNT(*) as c FROM "LiveSession"${where}${andWhere} status = 'stale'`).get(...params) as { c: number } | undefined)?.c || 0;

    const nasCounts = await db.query(
...
app.get('/api/live-sessions/:username', async (c) => {
  try {
    const username = c.req.param('username');
    const row = await db.query('SELECT * FROM "LiveSession" WHERE username = ?').get(username) as Record<string, unknown> | undefined;
...
app.post('/api/live-sessions', async (c) => {
  try {
    ...
    db.query(
      ...
      id, await resolveTenantAndProperty(tenantId, propertyId).tenantId, await resolveTenantAndProperty(tenantId, propertyId).propertyId, acctSessionId || id, username, userId || null, planId || null,
...
app.put('/api/live-sessions/:id', async (c) => {
  try {
    ...
    const existing = await db.query('SELECT * FROM "LiveSession" WHERE id = ?').get(id) as Record<string, unknown> | undefined;
    ...
    params.push(id);
    await db.query(`UPDATE "LiveSession" SET ${updates.join(', ')} WHERE id = ?`).run(...params);
...
app.post('/api/live-sessions/end-local', async (c) => {
  try {
    ...
    if (sessionId) {
      liveSession = await db.query('SELECT id, acctSessionId, username FROM "LiveSession" WHERE id = ? AND status = \'active\'').get(sessionId) as Record<string, unknown> | null;
    }
    if (!liveSession && acctSessionId) {
      liveSession = await db.query('SELECT id, acctSessionId, username FROM "LiveSession" WHERE acctSessionId = ? AND status = \'active\'').get(acctSessionId) as Record<string, unknown> | null;
    }
    if (!liveSession && sessionId && sessionId.startsWith('ls_')) {
      const bareId = sessionId.slice(3);
      liveSession = await db.query('SELECT id, acctSessionId, username FROM "LiveSession" WHERE acctSessionId = ? AND status = \'active\'').get(bareId) as Record<string, unknown> | null;
    }
...
app.get('/api/fap-policies', async (c) => {
  try {
    const propertyId = c.req.query('propertyId') || '';
...
app.get('/api/web-categories', async (c) => {
  try {
    const propertyId = c.req.query('propertyId') || '';
...
app.post('/api/web-categories', async (c) => {
  try {
    ...
    db.query(
      ...
      id, await resolveTenantAndProperty(tenantId, propertyId).tenantId, await resolveTenantAndProperty(tenantId, propertyId).propertyId, name, description || null,
...
app.put('/api/web-categories/:id', async (c) => {
  try {
    const id = c.req.param('id');
    const body = await c.req.json();
    const existing = await db.query('SELECT * FROM "WebCategory" WHERE id = ?').get(id) as Record<string, unknown> | undefined;
...
app.delete('/api/web-categories/:id', async (c) => {
  try {
    const id = c.req.param('id');
    await db.query('DELETE FROM "WebCategorySchedule" WHERE webCategoryId = ?').run(id);
    const result = await db.query('DELETE FROM "WebCategory" WHERE id = ?').run(id);
    if (result.rowCount === 0) return c.json({ success: false, error: 'Web category not found' }, 404);
...
app.get('/api/web-categories/:id/schedules', async (c) => {
  try {
    const webCategoryId = c.req.param('id');
    const rows = await db.query('SELECT * FROM "WebCategorySchedule" WHERE webCategoryId = ? ORDER BY orderIndex ASC').all(webCategoryId) as Record<string, unknown>[];
    return c.json({ success: true, data: rows, total: rows.length });
  } catch (error) {
    return c.json({ success: false, error: String(error) }, 500);
  }
...
app.post('/api/web-categories/:id/schedules', async (c) => {
  try {
    ...
    db.query(
      ...
      id, await resolveTenantAndProperty(tenantId, propertyId).tenantId, await resolveTenantAndProperty(tenantId, propertyId).propertyId, webCategoryId, scheduleAccessId || null,
...
app.put('/api/web-categories/schedules/:id', async (c) => {
  try {
    const id = c.req.param('id');
    const body = await c.req.json();
    const existing = await db.query('SELECT * FROM "WebCategorySchedule" WHERE id = ?').get(id) as Record<string, unknown> | undefined;
    if (!existing) return c.json({ success: false, error: 'Schedule not found' }, 404);
...
app.delete('/api/web-categories/schedules/:id', async (c) => {
  try {
    const id = c.req.param('id');
    const result = await db.query('DELETE FROM "WebCategorySchedule" WHERE id = ?').run(id);
    if (result.rowCount === 0) return c.json({ success: false, error: 'Schedule not found' }, 404);
...
app.get('/api/user-status-history', async (c) => {
  try {
    const propertyId = c.req.query('propertyId') || '';
    const username = c.req.query('username') || '';
    const startDate = c.req.query('startDate') || '';
...
app.post('/api/user-status-history', async (c) => {
  try {
    ...
    db.query(
      ...
      id, await resolveTenantAndProperty(tenantId, propertyId).tenantId, await resolveTenantAndProperty(tenantId, propertyId).propertyId, username || '', userId || null,
...
app.get('/api/nas-health', async (c) => {
  try {
    const propertyId = c.req.query('propertyId') || '';
    const nasIpAddress = c.req.query('nasIpAddress') || '';
...
app.get('/api/nas-health/current', async (c) => {
  try {
    const propertyId = c.req.query('propertyId') || '';
...
app.post('/api/nas-health/check', async (c) => {
  try {
    ...
    const nasClient = await db.query('SELECT * FROM nas WHERE nasname = ?').get(nasIpAddress) as Record<string, unknown> | undefined;
...
app.get('/api/nas-health/stats', async (c) => {
  try {
    const propertyId = c.req.query('propertyId') || '';
    const where = propertyId ? ' WHERE propertyId = ?' : '';
    const andWhere = propertyId ? ' AND' : ' WHERE';
    ...
    const total = (await db.query(`SELECT COUNT(*) as c FROM "NasHealthLog"${where}`).get(...params) as { c: number } | undefined)?.c || 0;
    const online = (await db.query(`SELECT COUNT(DISTINCT nasIpAddress) as c FROM "NasHealthLog"${where}${andWhere} isOnline = 1`).get(...params) as { c: number } | undefined)?.c || 0;
    const offline = (await db.query(`SELECT COUNT(DISTINCT nasIpAddress) as c FROM "NasHealthLog"${where}${andWhere} isOnline = 0`).get(...params) as { c: number } | undefined)?.c || 0;
    const avgLatency = (await db.query(`SELECT AVG(avgLatencyMs) as a FROM "NasHealthLog"${where}${andWhere} isOnline = 1 AND avgLatencyMs IS NOT NULL`).get(...params) as { a: number } | undefined)?.a || 0;
...
app.get('/api/bw-policy-details', async (c) => {
  try {
    ...
    const rows = await db.query('SELECT * FROM "BandwidthPolicyDetail" WHERE bandwidthPolicyId = ? ORDER BY priority ASC').all(bandwidthPolicyId) as Record<string, unknown>[];
    return c.json({ success: true, data: rows, total: rows.length });
  } catch (error) {
    return c.json({ success: false, error: String(error) }, 500);
  }
...
app.post('/api/bw-policy-details', async (c) => {
  try {
    ...
    db.query(
      ...
      id, await resolveTenantId(tenantId), bandwidthPolicyId, scheduleAccessId || null,
...
app.put('/api/bw-policy-details/:id', async (c) => {
  try {
    const id = c.req.param('id');
    const body = await c.req.json();
    const existing = await db.query('SELECT * FROM "BandwidthPolicyDetail" WHERE id = ?').get(id) as Record<string, unknown> | undefined;
...
app.delete('/api/bw-policy-details/:id', async (c) => {
  try {
    const id = c.req.param('id');
    const result = await db.query('DELETE FROM "BandwidthPolicyDetail" WHERE id = ?').run(id);
    if (result.rowCount === 0) return c.json({ success: false, error: 'BW policy detail not found' }, 404);
    ...
    for (const session of activeSessions) {
      ...
      if (usedMb >= limitMb && policy.switchOverBwPolicyId) {
        ...
        if (!recentCoa) {
          ...
          const nas = await lookupNAS(session.nasIpAddress as string);
          if (nas) {
            const vendor = normalizeVendor(nas.type);
...
// Auto-setup FreeRADIUS SQL module on startup (configure radiusd to read from PostgreSQL)
...
// ============================================================================
// LDAP Module Configuration Endpoints
// ============================================================================

const MODS_ENABLED_PATH = path.join(RADIUS_CONFIG_PATH, 'mods-enabled');
const MODS_AVAILABLE_PATH = path.join(RADIUS_CONFIG_PATH, 'mods-available');
const LDAP_MOD_AVAILABLE = path.join(MODS_AVAILABLE_PATH, 'ldap');
const LDAP_MOD_ENABLED = path.join(MODS_ENABLED_PATH, 'ldap');
const INNER_TUNNEL_PATH = path.join(RADIUS_CONFIG_PATH, 'sites-available', 'inner-tunnel');
const INNER_TUNNEL_ENABLED = path.join(RADIUS_CONFIG_PATH, 'sites-enabled', 'inner-tunnel');

/**
 * Generate FreeRADIUS rlm_ldap module configuration from StaySuite LDAP config.
 * This creates the mods-enabled/ldap file content.
 */
function generateLDAPModuleConfig(config: {
  serverUrl: string;
  baseDn: string;
  bindDn: string;
  bindPassword: string;
  ...
  try {
    ...
    isLdaps = url.protocol === 'ldaps:';
    if (isLdaps && !url.port) port = 636;
  } catch {
    // Fallback: treat as hostname
    server = config.serverUrl.split(':')[0];
  }

  const tlsSection = config.useTls || isLdaps ? `
    tls {
        require_cert    = "allow"
        start_tls       = ${config.useStartTls ? 'yes' : 'no'}
    }
` : '';

  const updateSection = config.mschapAuth ? `
    update {
        control:NT-Password = "\\%{mschap:NT-Password}"
    }
` : '';

  const groupFilterSection = config.filterGroup
    ? `\n        group_membership_filter = "(|(&(objectClass=Group)(member=%{control:Ldap-UserDn}))(&(objectClass=Group)(memberOf:1.2.840.113556.1.4.1941:=%{control:Ldap-UserDn})))"\n        group_name_attribute = "cn"`
    : `\n        group_membership_attribute = "${config.groupAttr}"`;

  return `# Auto-generated by StaySuite HospitalityOS — DO NOT EDIT MANUALLY
# Last updated: ${new Date().toISOString()}

ldap {
    server = "${server}"
    port = ${port}
    identity = "${config.bindDn}"
...
${groupFilterSection}
    ...
    options {
        ...
        chase_referrals = no
        rebind = yes
        net_timeout = ${config.networkTimeout}
        timeout = ${config.timeout}
        res_timeout = ${config.networkTimeout}
        srv_timelimit = ${config.timeout}
        ldap_connection_timeout = ${config.timeout}
    }

    pool {
        start = ${config.poolMin}
        min = ${config.poolMin}
        max = ${config.poolMax}
        spare = 3
        uses = 0
        lifetime = 0
        idle_timeout = 60
        connect_timeout = ${config.networkTimeout}
    }
}
`;
}
...
/**
 * Toggle LDAP module in FreeRADIUS authorize{} and authenticate{} sections.
 * When enabling: adds "-ldap" to authorize, adds "Auth-Type LDAP { ldap }" to authenticate,
 *   optionally configures ntlm_auth in mschap module.
 * When disabling: removes LDAP from both sections, restores to clean state.
 */
async function toggleLDAPInSites(enabled: boolean, config?: {
  mschapAuth: boolean;
  ntlmAuthPath?: string;
  winbindDomain?: string;
  filterGroup?: string;
  ...
  // ── 1. Toggle LDAP module symlink ──
  try {
    if (enabled) {
      // Ensure symlink exists
      try {
        const existingTarget = await fs.readlink(LDAP_MOD_ENABLED);
        if (existingTarget) {
          results.push('mods-enabled/ldap: already linked');
        }
      } catch {
        // Symlink doesn't exist — create it
        const relPath = path.relative(MODS_ENABLED_PATH, LDAP_MOD_AVAILABLE);
        await fs.symlink(relPath, LDAP_MOD_ENABLED);
        results.push('mods-enabled/ldap: ENABLED (symlinked)');
      }
    } else {
      // Remove symlink if it exists
      try {
        await fs.unlink(LDAP_MOD_ENABLED);
        results.push('mods-enabled/ldap: DISABLED (symlink removed)');
      } catch {
        results.push('mods-enabled/ldap: already disabled');
      }
    }
  } catch (err) {
    results.push(`mods-enabled/ldap: ERROR - ${String(err)}`);
    return { results, success: false };
  }

  // ── 2. Toggle LDAP in sites-available/default ──
  try {
    let content = await fs.readFile(SITES_DEFAULT_PATH, 'utf-8');

    if (enabled) {
      // Add -ldap to authorize section (after "-sql" or in the authorize block)
      if (!content.match(/^\s*-ldap\s*$/m)) {
        // Find the authorize section and add -ldap before -sql if present
        if (content.match(/^\s*-sql\s*$/m)) {
          content = content.replace(/^(\s*)-sql\s*$/m, '$1-ldap\n$1-sql');
          results.push('sites-available/default: -ldap added to authorize{} (before -sql)');
        } else {
          // Add -ldap at the end of authorize section
          content = content.replace(
            /^(authorize\s*\{)/m,
            '$1\n\t-ldap'
          );
          results.push('sites-available/default: -ldap added to authorize{}');
        }
      } else {
        results.push('sites-available/default: -ldap already in authorize{}');
      }

      // Add Auth-Type LDAP block to authenticate section
      if (!content.match(/Auth-Type\s+LDAP\s*\{[\s\S]*?ldap[\s\S]*?\}/m)) {
        // Find authenticate section and add Auth-Type LDAP block
        content = content.replace(
          /^(authenticate\s*\{)/m,
          `$1\n\tAuth-Type LDAP {\n\t\tldap\n\t}`
        );
        results.push('sites-available/default: Auth-Type LDAP block added to authenticate{}');
      } else {
        results.push('sites-available/default: Auth-Type LDAP block already present');
      }

      // Add Ldap-Group check in post-auth (to map AD groups to RADIUS attributes)
      if (!content.match(/Ldap-Group/m)) {
        content = content.replace(
          /^(post-auth\s*\{)/m,
          `$1\n\t# StaySuite: LDAP group mapping\n\tLdap-Group\n\t\t# Map AD groups to RADIUS reply attributes below`
        );
        results.push('sites-available/default: Ldap-Group added to post-auth{}');
      }
    } else {
      // Remove -ldap from authorize
      content = content.replace(/^\s*-ldap\s*\n?/gm, '');
      results.push('sites-available/default: -ldap removed from authorize{}');

      // Remove Auth-Type LDAP block
      content = content.replace(/\n?\s*Auth-Type\s+LDAP\s*\{[\s\S]*?ldap[\s\S]*?\}/g, '');
      results.push('sites-available/default: Auth-Type LDAP block removed');

      // Remove Ldap-Group from post-auth
      content = content.replace(/\n?\s*#\s*StaySuite:\s*LDAP group mapping\n?\s*Ldap-Group\n?\s*#\s*Map AD groups to RADIUS reply attributes below\n?/g, '');
      results.push('sites-available/default: Ldap-Group removed from post-auth{}');
    }

    await writeRadiusFile(SITES_DEFAULT_PATH, content);
  } catch (err) {
    results.push(`sites-available/default: ERROR - ${String(err)}`);
    // Don't return early — try to continue
  }

  // ── 3. Configure ntlm_auth in mschap module (if MS-CHAPv2 enabled) ──
  try {
    const mschapPath = path.join(MODS_AVAILABLE_PATH, 'mschap');
    const mschapEnabledPath = path.join(MODS_ENABLED_PATH, 'mschap');
    const targetPath = fsSync.existsSync(mschapEnabledPath) ? mschapEnabledPath : mschapPath;

    let mschapContent = await fs.readFile(targetPath, 'utf-8');

    if (enabled && config?.mschapAuth && config?.ntlmAuthPath) {
      // Remove any existing StaySuite-managed ntlm_auth config
      mschapContent = mschapContent.replace(/\n?# StaySuite: ntlm_auth[\s\S]*?(?=\n\S|\n$)/gm, '');

      const ntlmConfig = config.winbindDomain
        ? `\n\t# StaySuite: ntlm_auth for AD/LDAP MS-CHAPv2\n\tntlm_auth = "${config.ntlmAuthPath} --request-nt-key --domain=${config.winbindDomain}"`
        : `\n\t# StaySuite: ntlm_auth for AD/LDAP MS-CHAPv2\n\tntlm_auth = "${config.ntlmAuthPath} --request-nt-key"`;

      // Insert into the mschap module (before the closing brace)
      mschapContent = mschapContent.replace(
        /^(\s*\})\s*$/m,
        `${ntlmConfig}\n$1`
      );

      await writeRadiusFile(targetPath, mschapContent);
      results.push(`mschap: ntlm_auth configured for AD (path=${config.ntlmAuthPath})`);
    } else if (!enabled || !config?.mschapAuth) {
      // Remove StaySuite-managed ntlm_auth config
      const before = mschapContent;
      mschapContent = mschapContent.replace(/\n?# StaySuite: ntlm_auth[\s\S]*?(?=\n\S|\n\})/gm, '');
      if (mschapContent !== before) {
        await writeRadiusFile(targetPath, mschapContent);
        results.push('mschap: ntlm_auth config removed');
      }
    }
  } catch (err) {
    results.push(`mschap module: WARNING - ${String(err)}`);
  }

  // ── 4. Reload FreeRADIUS ──
  const reloaded = await reloadRadius();
  results.push(`FreeRADIUS reload: ${reloaded ? 'SUCCESS' : 'FAILED (manual restart needed)'}`);

  return { results, success: true };
}

app.post('/api/service/apply-ldap-config', async (c) => {
  try {
    const body = await c.req.json();
    const {
      enabled = false,
    ...
    // ── 1. Generate and write LDAP module config ──
    try {
      const ldapConfig = generateLDAPModuleConfig({
        serverUrl, baseDn, bindDn, bindPassword, searchFilter,
        useTls, useStartTls, timeout, networkTimeout, poolMin, poolMax,
        filterGroup, usernameAttr, groupAttr, mschapAuth, ntlmAuthPath, winbindDomain,
      });
      await writeRadiusFile(LDAP_MOD_AVAILABLE, ldapConfig);
      results.push('mods-available/ldap: configuration written');
    } catch (err) {
      results.push(`mods-available/ldap: FAILED - ${String(err)}`);
      log.error('Failed to write LDAP module config', { error: String(err) });
    }

    // ── 2. Toggle LDAP in sites and reload ──
    const toggleResult = await toggleLDAPInSites(enabled, {
      mschapAuth, ntlmAuthPath, winbindDomain, filterGroup,
    });
    results.push(...toggleResult.results);

    if (!toggleResult.success && enabled) {
      return c.json({
        success: false,
        message: 'LDAP module toggle failed — check FreeRADIUS logs',
        results,
      });
    }
    ...
    return c.json({
      success: true,
      message: enabled ? 'LDAP module enabled and configured in FreeRADIUS' : 'LDAP module disabled in FreeRADIUS',
      results,
    });
  } catch (error) {
    log.error('Failed to apply LDAP configuration', { error: String(error) });
    return c.json({
      success: false,
      error: `Failed to apply LDAP configuration: ${String(error)}`,
    }, 500);
  }
});

app.post('/api/service/ldap-diagnostics', async (c) => {
  try {
    const results: Record<string, string | boolean | number> = {};

    // Check if ldap module exists
    results.ldapModuleAvailable = fsSync.existsSync(LDAP_MOD_AVAILABLE);
    results.ldapModuleEnabled = fsSync.existsSync(LDAP_MOD_ENABLED);

    // Check if it's symlinked correctly
    if (results.ldapModuleEnabled) {
      try {
        const target = await fs.readlink(LDAP_MOD_ENABLED);
        results.ldapSymlinkTarget = target;
      } catch {
        results.ldapSymlinkTarget = 'ERROR: not a symlink';
      }
    }

    // Check if ntlm_auth binary exists
    results.ntlmAuthExists = fsSync.existsSync('/usr/bin/ntlm_auth');
    try {
      const { stdout } = await execAsync('/usr/bin/ntlm_auth --version 2>&1 | head -1 || echo "not found"', { timeout: 5000 });
      results.ntlmAuthVersion = stdout.trim();
    } catch {
      results.ntlmAuthVersion = 'not installed';
    }

    // Check if LDAP is referenced in sites-available/default
    try {
      const defaultContent = await fs.readFile(SITES_DEFAULT_PATH, 'utf-8');
      results.ldapInAuthorize = /^\s*-ldap\s*$/m.test(defaultContent);
      results.authTypeLdapBlock = /Auth-Type\s+LDAP\s*\{/m.test(defaultContent);
      results.ldapInPostAuth = /Ldap-Group/m.test(defaultContent);
    } catch {
      results.ldapInAuthorize = false;
      results.authTypeLdapBlock = false;
      results.ldapInPostAuth = false;
    }

    // Check FreeRADIUS status
    const status = await checkRadiusStatus();
    results.radiusInstalled = status.installed;
    results.radiusRunning = status.running;
