...
async function main() {
  ...
  try {
    await prisma.pricingRule.createMany({
      data: [
        // ── Basic Adjustments ──────────────────────────────────────
        // pr-1: markup — general price increase
        { id: uuid('pr-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Peak Season Markup', type: 'markup', description: '10% general markup during peak season for all room types', value: 10, valueType: 'percentage', conditions: JSON.stringify({}), priority: 4, isActive: true, effectiveFrom: new Date(today.getTime() - 30 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-1', 'roomtype-2', 'roomtype-3', 'roomtype-4']), appliedCount: 89, lastAppliedAt: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000) },
        // pr-2: markdown — off-season discount
        { id: uuid('pr-2'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Monsoon Clearance Sale', type: 'markdown', description: '15% markdown across all rooms during monsoon', value: 15, valueType: 'percentage', conditions: JSON.stringify({ daysOfWeek: ['Mon', 'Tue', 'Wed', 'Thu'] }), priority: 3, isActive: true, effectiveFrom: new Date(today.getTime() - 60 * 24 * 60 * 60 * 1000), effectiveTo: new Date(today.getTime() + 60 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-1', 'roomtype-2']), appliedCount: 34, lastAppliedAt: new Date(today.getTime() - 3 * 24 * 60 * 60 * 1000) },
        // pr-3: discount_percentage — flat % off
        { id: uuid('pr-3'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Corporate Rate Discount', type: 'discount_percentage', description: '12% percentage discount for corporate bookings', value: 12, valueType: 'percentage', conditions: JSON.stringify({ bookingChannel: ['direct', 'phone'] }), priority: 6, isActive: true, effectiveFrom: new Date(today.getTime() - 90 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-2', 'roomtype-3']), appliedCount: 56, lastAppliedAt: new Date() },
        // pr-4: discount_fixed — flat ₹ off
        { id: uuid('pr-4'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Weekday Flash Sale', type: 'discount_fixed', description: 'Flat ₹500 off on weekday bookings', value: 500, valueType: 'fixed', conditions: JSON.stringify({ daysOfWeek: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'] }), priority: 2, isActive: true, effectiveFrom: new Date(today.getTime() - 15 * 24 * 60 * 60 * 1000), effectiveTo: new Date(today.getTime() + 15 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-1']), appliedCount: 18 },
        // pr-5: surcharge_percentage — % extra
        { id: uuid('pr-5'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'New Year Eve Surcharge', type: 'surcharge_percentage', description: '30% surcharge for New Year Eve week', value: 30, valueType: 'percentage', conditions: JSON.stringify({}), priority: 15, isActive: true, effectiveFrom: new Date(today.getTime() - 10 * 24 * 60 * 60 * 1000), effectiveTo: new Date(today.getTime() + 20 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-3', 'roomtype-4']), appliedCount: 5 },
        // pr-6: surcharge_fixed — flat extra ₹
        { id: uuid('pr-6'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Festival Gala Dinner Add-on', type: 'surcharge_fixed', description: 'Flat ₹2000 surcharge during Durga Puja for gala dinner inclusion', value: 2000, valueType: 'fixed', conditions: JSON.stringify({}), priority: 8, isActive: true, effectiveFrom: new Date(today.getTime() - 45 * 24 * 60 * 60 * 1000), effectiveTo: new Date(today.getTime() + 15 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-1', 'roomtype-2', 'roomtype-3', 'roomtype-4']), appliedCount: 10 },

        // ── Time-Based ─────────────────────────────────────────────
        // pr-7: early_bird
        { id: uuid('pr-7'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Plan Ahead & Save', type: 'early_bird', description: '15% off for bookings made 21+ days in advance', value: 15, valueType: 'percentage', conditions: JSON.stringify({ advanceBookingDaysMin: 21 }), priority: 5, isActive: true, effectiveFrom: new Date(today.getTime() - 180 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-1', 'roomtype-2', 'roomtype-3']), appliedCount: 67, lastAppliedAt: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000) },
        // pr-8: last_minute
        { id: uuid('pr-8'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Last Minute Deal', type: 'last_minute', description: '20% off for bookings within 48 hours of check-in', value: 20, valueType: 'percentage', conditions: JSON.stringify({ advanceBookingDaysMax: 2 }), priority: 7, isActive: true, effectiveFrom: new Date(today.getTime() - 60 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-1', 'roomtype-2']), appliedCount: 23, lastAppliedAt: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000) },
        // pr-9: advance_booking
        { id: uuid('pr-9'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Summer Early Booking', type: 'advance_booking', description: '8% discount for summer bookings made 30-90 days in advance', value: 8, valueType: 'percentage', conditions: JSON.stringify({ advanceBookingDaysMin: 30, advanceBookingDaysMax: 90 }), priority: 4, isActive: true, effectiveFrom: new Date(today.getTime() - 120 * 24 * 60 * 60 * 1000), effectiveTo: new Date(today.getTime() + 180 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-1', 'roomtype-2', 'roomtype-3', 'roomtype-4']), appliedCount: 41 },
        // pr-10: seasonal
        { id: uuid('pr-10'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Durga Puja Season Premium', type: 'seasonal', description: '25% premium during Durga Puja festival season', value: 25, valueType: 'percentage', conditions: JSON.stringify({ startDate: '2025-10-01', endDate: '2025-10-24' }), priority: 10, isActive: true, effectiveFrom: new Date(today.getTime() - 60 * 24 * 60 * 60 * 1000), effectiveTo: new Date(today.getTime() + 90 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-1', 'roomtype-2', 'roomtype-3', 'roomtype-4']), appliedCount: 8 },
        // pr-11: weekend
        { id: uuid('pr-11'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Weekend Surge', type: 'weekend', description: '15% markup on weekends for all room types', value: 15, valueType: 'percentage', conditions: JSON.stringify({ daysOfWeek: ['Sat', 'Sun'] }), priority: 5, isActive: true, effectiveFrom: new Date(today.getTime() - 90 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-1', 'roomtype-2', 'roomtype-3']), appliedCount: 78, lastAppliedAt: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000) },

        // ── Stay-Based ────────────────────────────────────────────
        // pr-12: long_stay
        { id: uuid('pr-12'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Extended Stay Discount', type: 'long_stay', description: '12% discount for stays of 7+ nights', value: 12, valueType: 'percentage', conditions: JSON.stringify({ minNights: 7 }), priority: 3, isActive: true, effectiveFrom: new Date(today.getTime() - 120 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-1', 'roomtype-2']), appliedCount: 29, lastAppliedAt: new Date(today.getTime() - 4 * 24 * 60 * 60 * 1000) },

        // ── Guest-Based ───────────────────────────────────────────
        // pr-13: occupancy
        { id: uuid('pr-13'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Extra Guest Surcharge', type: 'occupancy', description: '₹800 per extra guest beyond 2 adults', value: 800, valueType: 'fixed', conditions: JSON.stringify({ minOccupancy: 3 }), priority: 2, isActive: true, effectiveFrom: new Date(today.getTime() - 200 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-1', 'roomtype-2', 'roomtype-3', 'roomtype-4']), appliedCount: 112, lastAppliedAt: new Date() },

        // ── Promotions ────────────────────────────────────────────
        // pr-14: promo_code
        { id: uuid('pr-14'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Welcome Back Offer', type: 'promo_code', description: '18% off for returning guests using promo code STAY2025', value: 18, valueType: 'percentage', conditions: JSON.stringify({ promoCode: 'STAY2025', promoMaxUses: 200 }), priority: 8, isActive: true, effectiveFrom: new Date(today.getTime() - 30 * 24 * 60 * 60 * 1000), effectiveTo: new Date(today.getTime() + 120 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-1', 'roomtype-2', 'roomtype-3']), appliedCount: 43, lastAppliedAt: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000) },
        // pr-15: channel
        { id: uuid('pr-15'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'OTA Commission Offset', type: 'channel', description: '12% markup on OTA bookings to offset commission', value: 12, valueType: 'percentage', conditions: JSON.stringify({ bookingChannel: ['booking_com', 'expedia', 'agoda'] }), priority: 6, isActive: true, effectiveFrom: new Date(today.getTime() - 60 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-1', 'roomtype-2', 'roomtype-3', 'roomtype-4']), appliedCount: 87, lastAppliedAt: new Date() },

        // ── Legacy type (kept for backward compat) ────────────────
        // pr-16: dynamic — generic dynamic pricing (kept for backward compat)
        { id: uuid('pr-16'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'AI Dynamic Pricing', type: 'dynamic', description: 'Auto-adjusted by RevPAR optimizer', value: 5, valueType: 'percentage', conditions: JSON.stringify({}), priority: 1, isActive: false, effectiveFrom: new Date(today.getTime() - 180 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-1', 'roomtype-2', 'roomtype-3', 'roomtype-4']), appliedCount: 0 },

        // ── Property-2 rules ──────────────────────────────────────
        { id: uuid('pr-17'), tenantId: uuid('tenant-1'), propertyId: uuid('property-2'), name: 'Hill Station Weekend Premium', type: 'weekend', description: '20% weekend markup for mountain property', value: 20, valueType: 'percentage', conditions: JSON.stringify({ daysOfWeek: ['Sat', 'Sun'] }), priority: 5, isActive: true, effectiveFrom: new Date(today.getTime() - 60 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-5', 'roomtype-6']), appliedCount: 32, lastAppliedAt: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000) },
        { id: uuid('pr-18'), tenantId: uuid('tenant-1'), propertyId: uuid('property-2'), name: 'Off-Season Markdown', type: 'markdown', description: '20% off during off-season months', value: 20, valueType: 'percentage', conditions: JSON.stringify({}), priority: 3, isActive: true, effectiveFrom: new Date(today.getTime() - 30 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-5']), appliedCount: 15 },
        { id: uuid('pr-19'), tenantId: uuid('tenant-1'), propertyId: uuid('property-2'), name: 'Valley View Early Bird', type: 'early_bird', description: '10% off for 14+ day advance bookings', value: 10, valueType: 'percentage', conditions: JSON.stringify({ advanceBookingDaysMin: 14 }), priority: 4, isActive: true, effectiveFrom: new Date(today.getTime() - 90 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-6']), appliedCount: 22 },

        // ── Property-2 additional rule type samples ──────────────
        // pr-20: last_minute for property-2
        { id: uuid('pr-20'), tenantId: uuid('tenant-1'), propertyId: uuid('property-2'), name: 'Mountain Last Minute Escape', type: 'last_minute', description: '25% markup for same-day bookings (high demand)', value: 25, valueType: 'percentage', conditions: JSON.stringify({ advanceBookingDaysMax: 1 }), priority: 8, isActive: true, effectiveFrom: new Date(today.getTime() - 30 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-5', 'roomtype-6']), appliedCount: 9 },
        // pr-21: advance_booking for property-2
        { id: uuid('pr-21'), tenantId: uuid('tenant-1'), propertyId: uuid('property-2'), name: 'Monsoon Advance Saver', type: 'advance_booking', description: '12% off for monsoon bookings made 30-60 days ahead', value: 12, valueType: 'percentage', conditions: JSON.stringify({ advanceBookingDaysMin: 30, advanceBookingDaysMax: 60 }), priority: 5, isActive: true, effectiveFrom: new Date(today.getTime() - 45 * 24 * 60 * 60 * 1000), effectiveTo: new Date(today.getTime() + 120 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-5', 'roomtype-6']), appliedCount: 14 },
        // pr-22: seasonal for property-2
        { id: uuid('pr-22'), tenantId: uuid('tenant-1'), propertyId: uuid('property-2'), name: 'Summer Peak Premium', type: 'seasonal', description: '30% premium during summer months', value: 30, valueType: 'percentage', conditions: JSON.stringify({ months: [4, 5, 6] }), priority: 12, isActive: true, effectiveFrom: new Date(today.getTime() - 60 * 24 * 60 * 60 * 1000), effectiveTo: new Date(today.getTime() + 90 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-5', 'roomtype-6']), appliedCount: 11 },
        // pr-23: long_stay for property-2
        { id: uuid('pr-23'), tenantId: uuid('tenant-1'), propertyId: uuid('property-2'), name: 'Retreat Long Stay Package', type: 'long_stay', description: '18% off for stays of 14+ nights', value: 18, valueType: 'percentage', conditions: JSON.stringify({ minNights: 14 }), priority: 6, isActive: true, effectiveFrom: new Date(today.getTime() - 60 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-5', 'roomtype-6']), appliedCount: 7 },
        // pr-24: occupancy for property-2
        { id: uuid('pr-24'), tenantId: uuid('tenant-1'), propertyId: uuid('property-2'), name: 'Family Room Extra Guest', type: 'occupancy', description: '₹600 per extra guest beyond 3 adults', value: 600, valueType: 'fixed', conditions: JSON.stringify({ minOccupancy: 4 }), priority: 3, isActive: true, effectiveFrom: new Date(today.getTime() - 90 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-5']), appliedCount: 18 },
        // pr-25: promo_code for property-2
        { id: uuid('pr-25'), tenantId: uuid('tenant-1'), propertyId: uuid('property-2'), name: 'Honeymoon Special', type: 'promo_code', description: '22% off for honeymoon package with code LOVE2025', value: 22, valueType: 'percentage', conditions: JSON.stringify({ promoCode: 'LOVE2025', promoMaxUses: 50 }), priority: 10, isActive: true, effectiveFrom: new Date(today.getTime() - 15 * 24 * 60 * 60 * 1000), effectiveTo: new Date(today.getTime() + 180 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-5', 'roomtype-6']), appliedCount: 8 },
        // pr-26: channel for property-2
        { id: uuid('pr-26'), tenantId: uuid('tenant-1'), propertyId: uuid('property-2'), name: 'Airbnb Host Fee Offset', type: 'channel', description: '15% markup on Airbnb to offset host service fee', value: 15, valueType: 'percentage', conditions: JSON.stringify({ bookingChannel: ['airbnb'] }), priority: 7, isActive: true, effectiveFrom: new Date(today.getTime() - 45 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-5', 'roomtype-6']), appliedCount: 33 },
        // pr-27: discount_percentage for property-2
        { id: uuid('pr-27'), tenantId: uuid('tenant-1'), propertyId: uuid('property-2'), name: 'Senior Citizen Discount', type: 'discount_percentage', description: '15% discount for senior citizen bookings', value: 15, valueType: 'percentage', conditions: JSON.stringify({ guestType: ['senior'] }), priority: 4, isActive: true, effectiveFrom: new Date(today.getTime() - 120 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-5', 'roomtype-6']), appliedCount: 19 },
        // pr-28: surcharge_fixed for property-2
        { id: uuid('pr-28'), tenantId: uuid('tenant-1'), propertyId: uuid('property-2'), name: 'Holiday Breakfast Buffet', type: 'surcharge_fixed', description: '₹1200 breakfast buffet surcharge on holidays', value: 1200, valueType: 'fixed', conditions: JSON.stringify({ daysOfWeek: ['Sun'] }), priority: 5, isActive: true, effectiveFrom: new Date(today.getTime() - 30 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-5', 'roomtype-6']), appliedCount: 21 },
        // pr-29: discount_fixed for property-2
        { id: uuid('pr-29'), tenantId: uuid('tenant-1'), propertyId: uuid('property-2'), name: 'Weekday Flat Discount', type: 'discount_fixed', description: 'Flat ₹300 off Monday-Thursday stays', value: 300, valueType: 'fixed', conditions: JSON.stringify({ daysOfWeek: ['Mon', 'Tue', 'Wed', 'Thu'] }), priority: 3, isActive: true, effectiveFrom: new Date(today.getTime() - 20 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-6']), appliedCount: 12 },
        // pr-30: markup for property-2
        { id: uuid('pr-30'), tenantId: uuid('tenant-1'), propertyId: uuid('property-2'), name: 'Festival Season Uplift', type: 'markup', description: '8% markup across all rooms during festivals', value: 8, valueType: 'percentage', conditions: JSON.stringify({}), priority: 6, isActive: true, effectiveFrom: new Date(today.getTime() - 10 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-5', 'roomtype-6']), appliedCount: 5 },
        // pr-31: surcharge_percentage for property-2
        { id: uuid('pr-31'), tenantId: uuid('tenant-1'), propertyId: uuid('property-2'), name: 'Diwali Festival Surcharge', type: 'surcharge_percentage', description: '25% surcharge during Diwali week', value: 25, valueType: 'percentage', conditions: JSON.stringify({}), priority: 14, isActive: true, effectiveFrom: new Date(today.getTime() - 5 * 24 * 60 * 60 * 1000), effectiveTo: new Date(today.getTime() + 25 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-5', 'roomtype-6']), appliedCount: 3 },
      ],
    });
    console.log('31 pricing rules seeded (all 15 types across 2 properties + 1 legacy)!');
  } catch (e: any) {
  ...
  try {
    await prisma.bookingAuditLog.createMany({
      data: [
        { id: uuid('bal-1'), bookingId: uuid('booking-1'), action: 'created', newStatus: 'confirmed', notes: 'Direct booking by guest Amit Mukherjee', performedBy: uuid('user-2'), performedAt: new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000) },
        { id: uuid('bal-2'), bookingId: uuid('booking-1'), action: 'modified', oldStatus: 'confirmed', newStatus: 'confirmed', notes: 'Room changed from 505 to 501 per guest request', performedBy: uuid('user-2'), performedAt: new Date(today.getTime() - 6 * 24 * 60 * 60 * 1000) },
        { id: uuid('bal-3'), bookingId: uuid('booking-1'), action: 'checked_in', oldStatus: 'confirmed', newStatus: 'checked_in', notes: 'Guest arrived at 2:15 PM. ID verified.', performedBy: uuid('user-2'), performedAt: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000) },
        { id: uuid('bal-4'), bookingId: uuid('booking-2'), action: 'created', newStatus: 'confirmed', notes: 'Direct booking by VIP Platinum guest Rahul Banerjee', performedBy: uuid('user-1'), performedAt: new Date(today.getTime() - 5 * 24 * 60 * 60 * 1000) },
        { id: uuid('bal-5'), bookingId: uuid('booking-2'), action: 'checked_in', oldStatus: 'confirmed', newStatus: 'checked_in', notes: 'Guest checked in at 1:30 PM. VIP welcome amenity arranged.', performedBy: uuid('user-1'), performedAt: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000) },
        ...
        { id: uuid('bal-7'), bookingId: uuid('booking-3'), action: 'modified', oldStatus: 'confirmed', newStatus: 'confirmed', notes: 'Added to group booking TCS Annual Offsite 2024 (group-1)', performedBy: uuid('user-1'), performedAt: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000) },
        { id: uuid('bal-8'), bookingId: uuid('booking-4'), action: 'created', newStatus: 'confirmed', notes: 'Direct booking by gold tier guest Vikram Singh. Presidential Suite requested.', performedBy: uuid('user-1'), performedAt: new Date(today.getTime() - 3 * 24 * 60 * 60 * 1000) },
        ...
        { id: uuid('bal-10'), bookingId: uuid('booking-6'), action: 'checked_in', oldStatus: 'confirmed', newStatus: 'checked_in', notes: 'Guest checked in at 3:00 PM. Room 305 assigned.', performedBy: uuid('user-2'), performedAt: new Date(today.getTime() - 3 * 24 * 60 * 60 * 1000) },
      ],
    });
  ...
  // ============================================================
  // CRITICAL MISSING FEATURES SEED DATA (9 modules)
  // ============================================================

  // ─── A1. Night Audit ─────────────────────────────────────────
  console.log('Seeding night audit data...');
  try {
    await prisma.nightAudit.createMany({
      data: [
        { id: uuid('na-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), auditDate: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000), businessDayDate: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000), status: 'completed', startedBy: uuid('user-2'), completedBy: uuid('user-1'), startedAt: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000 + 22 * 60 * 60 * 1000), completedAt: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000 + 23.5 * 60 * 60 * 1000), roomRevenue: 165000, fbRevenue: 45000, otherRevenue: 12000, totalRevenue: 222000, roomChargesPosted: 45, noShowsProcessed: 2, roomsReconciled: 98, discrepancies: 1, autoPostedAt: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000 + 22 * 60 * 60 * 1000), notes: '1 discrepancy in room 305 minibar count — resolved by Anita Roy.' },
        { id: uuid('na-2'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), auditDate: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000), businessDayDate: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000), status: 'completed', startedBy: uuid('user-1'), completedBy: uuid('user-1'), startedAt: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000 + 22 * 60 * 60 * 1000), completedAt: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000 + 23 * 60 * 60 * 1000), roomRevenue: 148000, fbRevenue: 38000, otherRevenue: 8500, totalRevenue: 194500, roomChargesPosted: 42, noShowsProcessed: 1, roomsReconciled: 97, discrepancies: 0, notes: 'Clean audit run.' },
        { id: uuid('na-3'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), auditDate: new Date(), businessDayDate: new Date(), status: 'in_progress', startedBy: uuid('user-2'), startedAt: new Date(today.getTime() - 30 * 60 * 60 * 1000), roomRevenue: 155000, fbRevenue: 42000, otherRevenue: 9000, totalRevenue: 206000, roomChargesPosted: 44, noShowsProcessed: 0, roomsReconciled: 96, discrepancies: 3, notes: 'In progress — 3 room discrepancies pending review.' },
      ],
    });
    await prisma.nightAuditStep.createMany({
      data: [
        ...
        { id: uuid('nas-1-4'), nightAuditId: uuid('na-1'), stepName: 'Verify Folio Balances', stepOrder: 4, status: 'completed', performedBy: uuid('user-1'), result: 'All folios balanced' },
        { id: uuid('nas-1-5'), nightAuditId: uuid('na-1'), stepName: 'Generate End-of-Day Report', stepOrder: 5, status: 'completed', performedBy: uuid('user-1'), result: 'EOD report generated' },
        { id: uuid('nas-3-1'), nightAuditId: uuid('na-3'), stepName: 'Verify Room Status', stepOrder: 1, status: 'completed', performedBy: uuid('user-2'), result: '96 rooms reconciled, 3 discrepancies' },
        { id: uuid('nas-3-2'), nightAuditId: uuid('na-3'), stepName: 'Post Room Charges', stepOrder: 2, status: 'completed', performedBy: uuid('user-2'), result: '44 room charges posted' },
    ...
    await prisma.nightAuditLog.createMany({
      data: [
    ...
    console.log('3 night audits with steps and logs seeded!');
  } catch (e: any) {
    console.log('Night audit seed error:', e.message);
  }

  // ─── A2. Travel Agent / City Ledger AR ───────────────────────
  console.log('Seeding travel agents and city ledger data...');
  try {
    await prisma.travelAgent.createMany({
      data: [
        { id: uuid('ta-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), agencyName: 'Thomas Cook India', code: 'TCI-001', contactPerson: 'Sanjay Verma', email: 'sanjay.v@thomascook.in', phone: '+91-33-22891234', address: '14 Park Street', city: 'Kolkata', country: 'India', taxId: 'GST-19AABCT1234A1Z5', commissionRate: 12, commissionType: 'percentage', creditLimit: 500000, currentBalance: 125000, paymentTerms: 'net_30', status: 'active', isActive: true, notes: 'Premium travel partner since 2019. Quarterly settlement preferred.' },
    ...
    await prisma.cityLedgerInvoice.createMany({
      data: [
        { id: uuid('cli-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), travelAgentId: uuid('ta-1'), accountName: 'Thomas Cook India', accountType: 'travel_agent', invoiceNumber: 'CL-2024-001', invoiceDate: new Date(today.getTime() - 25 * 24 * 60 * 60 * 1000), dueDate: new Date(today.getTime() - 5 * 24 * 60 * 60 * 1000), subtotal: 85000, tax: 15300, total: 100300, currency: 'INR', status: 'paid', paidAmount: 100300 },
        { id: uuid('cli-2'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), travelAgentId: uuid('ta-2'), accountName: 'MakeMyTrip Business', accountType: 'travel_agent', invoiceNumber: 'CL-2024-002', invoiceDate: new Date(today.getTime() - 20 * 24 * 60 * 60 * 1000), dueDate: new Date(today.getTime() - 5 * 24 * 60 * 60 * 1000), subtotal: 240000, tax: 43200, total: 283200, currency: 'INR', status: 'overdue', paidAmount: 200000 },
        { id: uuid('cli-3'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), travelAgentId: uuid('ta-4'), accountName: 'Expedia Affiliate', accountType: 'travel_agent', invoiceNumber: 'CL-2024-003', invoiceDate: new Date(today.getTime() - 15 * 24 * 60 * 60 * 1000), dueDate: new Date(today.getTime() + 15 * 24 * 60 * 60 * 1000), subtotal: 320000, tax: 57600, total: 377600, currency: 'INR', status: 'sent', paidAmount: 0 },
        { id: uuid('cli-4'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), travelAgentId: uuid('ta-2'), accountName: 'MakeMyTrip Business', accountType: 'travel_agent', invoiceNumber: 'CL-2024-004', invoiceDate: new Date(today.getTime() - 5 * 24 * 60 * 60 * 1000), dueDate: new Date(today.getTime() + 10 * 24 * 60 * 60 * 1000), subtotal: 175000, tax: 31500, total: 206500, currency: 'INR', status: 'draft', paidAmount: 0 },
        { id: uuid('cli-5'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), travelAgentId: uuid('ta-5'), accountName: 'Goibibo Corporate', accountType: 'travel_agent', invoiceNumber: 'CL-2024-005', invoiceDate: new Date(today.getTime() - 30 * 24 * 60 * 60 * 1000), dueDate: new Date(), subtotal: 65000, tax: 11700, total: 76700, currency: 'INR', status: 'partial', paidAmount: 40000 },
      ],
    });
    await prisma.cityLedgerItem.createMany({
      data: [
        { id: uuid('clitem-1'), invoiceId: uuid('cli-1'), description: '5 nights Deluxe Room - Booking.com referral', amount: 27500, quantity: 5, folioId: uuid('folio-2') },
        { id: uuid('clitem-2'), invoiceId: uuid('cli-1'), description: 'Room service and F&B charges', amount: 18500, quantity: 1 },
        { id: uuid('clitem-3'), invoiceId: uuid('cli-1'), description: 'Spa treatments (2 guests)', amount: 12000, quantity: 2 },
        { id: uuid('clitem-4'), invoiceId: uuid('cli-1'), description: 'Airport transfer services', amount: 8500, quantity: 3 },
        { id: uuid('clitem-5'), invoiceId: uuid('cli-2'), description: '10 nights Standard Room - MMT guests', amount: 35000, quantity: 10 },
        { id: uuid('clitem-6'), invoiceId: uuid('cli-2'), description: 'Conference room booking - 2 days', amount: 24000, quantity: 2 },
        { id: uuid('clitem-7'), invoiceId: uuid('cli-2'), description: 'Banquet dinner - 40 pax', amount: 120000, quantity: 1 },
        { id: uuid('clitem-8'), invoiceId: uuid('cli-2'), description: 'Additional F&B charges', amount: 61000, quantity: 1 },
      ],
    });
    await prisma.cityLedgerPayment.createMany({
      data: [
        { id: uuid('clpay-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), invoiceId: uuid('cli-1'), amount: 100300, paymentMethod: 'bank_transfer', reference: 'NEFT-TCI-78901', paidAt: new Date(today.getTime() - 8 * 24 * 60 * 60 * 1000), notes: 'Full payment via NEFT' },
        { id: uuid('clpay-2'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), invoiceId: uuid('cli-2'), amount: 200000, paymentMethod: 'bank_transfer', reference: 'NEFT-MMT-45678', paidAt: new Date(today.getTime() - 10 * 24 * 60 * 60 * 1000), notes: 'Partial payment — balance pending' },
        { id: uuid('clpay-3'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), invoiceId: uuid('cli-5'), amount: 40000, paymentMethod: 'upi', reference: 'UPI-GIB-12345', paidAt: new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000), notes: 'Partial payment received' },
      ],
    });
    console.log('5 travel agents, 5 city ledger invoices seeded!');
  } catch (e: any) {
    console.log('Travel agent / city ledger seed error:', e.message);
  }

  // ─── A3. Commission Management ───────────────────────────────
  console.log('Seeding commission data...');
  try {
    await prisma.commissionRule.createMany({
      data: [
        { id: uuid('crule-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Booking.com Standard', description: 'Standard 15% commission for all Booking.com bookings', sourceType: 'ota', commissionType: 'percentage', rate: 15, isActive: true, validFrom: new Date('2024-01-01') },
        { id: uuid('crule-2'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Expedia Partner', description: '18% commission for Expedia channel', sourceType: 'ota', commissionType: 'percentage', rate: 18, isActive: true, validFrom: new Date('2024-01-01') },
        { id: uuid('crule-3'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Thomas Cook Travel Agent', description: '12% commission for Thomas Cook India referrals', sourceType: 'travel_agent', sourceId: uuid('ta-1'), commissionType: 'percentage', rate: 12, isActive: true, validFrom: new Date('2024-01-01') },
        { id: uuid('crule-4'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Corporate Referral Bonus', description: 'Flat INR 500 per corporate referral booking', sourceType: 'corporate', commissionType: 'flat', fixedAmount: 500, isActive: true, validFrom: new Date('2024-04-01') },
        { id: uuid('crule-5'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Loyalty Member Referral', description: '8% commission for guest referral bookings from loyalty members', sourceType: 'referral', commissionType: 'percentage', rate: 8, isActive: true, validFrom: new Date('2024-01-01') },
        { id: uuid('crule-6'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Goibibo Dynamic', description: 'Tiered commission: 12% up to 5000, 16% above 5000 per booking', sourceType: 'ota', commissionType: 'tiered', rate: 12, maxAmount: 16, isActive: true, validFrom: new Date('2024-03-01') },
      ],
    });
    await prisma.commissionRecord.createMany({
      data: [
        { id: uuid('crec-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), ruleId: uuid('crule-1'), bookingId: uuid('booking-3'), sourceType: 'ota', sourceName: 'Booking.com', bookingAmount: 22000, commissionAmount: 3300, status: 'accrued', notes: 'Booking.com booking for Sneha Gupta' },
        { id: uuid('crec-2'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), ruleId: uuid('crule-1'), bookingId: uuid('booking-5'), sourceType: 'ota', sourceName: 'Booking.com', bookingAmount: 10500, commissionAmount: 1575, status: 'accrued', notes: 'Booking.com booking for Pooja Saha' },
        { id: uuid('crec-3'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), ruleId: uuid('crule-3'), bookingId: uuid('booking-1'), sourceType: 'travel_agent', sourceName: 'Thomas Cook India', bookingAmount: 16500, commissionAmount: 1980, status: 'invoiced', invoicedAt: new Date(today.getTime() - 25 * 24 * 60 * 60 * 1000), notes: 'Referred via Thomas Cook. Invoice CL-2024-001 raised.' },
        { id: uuid('crec-4'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), ruleId: uuid('crule-2'), bookingId: uuid('booking-2'), sourceType: 'ota', sourceName: 'Expedia', bookingAmount: 48000, commissionAmount: 8640, status: 'paid', invoicedAt: new Date(today.getTime() - 15 * 24 * 60 * 60 * 1000), paidAt: new Date(today.getTime() - 10 * 24 * 60 * 60 * 1000), notes: 'Expedia VIP booking. Commission settled via wire transfer.' },
        { id: uuid('crec-5'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), ruleId: uuid('crule-4'), bookingId: uuid('booking-6'), sourceType: 'corporate', sourceName: 'TCS Corporate', bookingAmount: 10500, commissionAmount: 500, status: 'accrued', notes: 'Corporate referral from TCS employee' },
      ],
    });
    console.log('6 commission rules, 5 commission records seeded!');
  } catch (e: any) {
    console.log('Commission seed error:', e.message);
  }

  // ─── A4. Minibar Management ──────────────────────────────────
  console.log('Seeding minibar data...');
  try {
    ...
    await prisma.minibarSetup.createMany({
      data: [
        { id: uuid('mbsetup-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), roomId: uuid('room-501'), itemJson: JSON.stringify([{ itemId: uuid('mbitem-1'), quantity: 2, threshold: 1 }, { itemId: uuid('mbitem-5'), quantity: 4, threshold: 2 }, { itemId: uuid('mbitem-6'), quantity: 2, threshold: 1 }, { itemId: uuid('mbitem-8'), quantity: 2, threshold: 1 }]), lastRestockedAt: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000), restockedBy: 'user-3' },
        { id: uuid('mbsetup-2'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), roomId: uuid('room-801'), itemJson: JSON.stringify([{ itemId: uuid('mbitem-1'), quantity: 2, threshold: 1 }, { itemId: uuid('mbitem-3'), quantity: 3, threshold: 1 }, { itemId: uuid('mbitem-5'), quantity: 4, threshold: 2 }, { itemId: uuid('mbitem-7'), quantity: 2, threshold: 1 }, { itemId: uuid('mbitem-9'), quantity: 2, threshold: 1 }, { itemId: uuid('mbitem-10'), quantity: 1, threshold: 0 }]), lastRestockedAt: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000), restockedBy: 'user-3' },
        { id: uuid('mbsetup-3'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), roomId: uuid('room-305'), itemJson: JSON.stringify([{ itemId: uuid('mbitem-1'), quantity: 2, threshold: 1 }, { itemId: uuid('mbitem-5'), quantity: 4, threshold: 2 }, { itemId: uuid('mbitem-6'), quantity: 1, threshold: 1 }]), lastRestockedAt: new Date(today.getTime() - 3 * 24 * 60 * 60 * 1000), restockedBy: 'user-3' },
      ],
    });
    await prisma.minibarConsumption.createMany({
      data: [
        { id: uuid('mbcons-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), bookingId: uuid('booking-1'), folioId: uuid('folio-1'), roomId: uuid('room-501'), itemId: uuid('mbitem-3'), itemName: 'Kingfisher Beer 330ml', quantity: 2, unitPrice: 180, totalPrice: 360, consumedAt: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000), postedToFolio: true, postedAt: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000), consumedBy: 'Guest Amit Mukherjee', notes: 'Evening drinks' },
        { id: uuid('mbcons-2'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), bookingId: uuid('booking-1'), folioId: uuid('folio-1'), roomId: uuid('room-501'), itemId: uuid('mbitem-7'), itemName: 'Mixed Nuts Pack', quantity: 1, unitPrice: 200, totalPrice: 200, consumedAt: new Date(today.getTime() - 12 * 60 * 60 * 1000), postedToFolio: false, consumedBy: 'Guest Amit Mukherjee' },
        { id: uuid('mbcons-3'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), bookingId: uuid('booking-2'), folioId: uuid('folio-2'), roomId: uuid('room-801'), itemId: uuid('mbitem-9'), itemName: 'Johnnie Walker Red Label 50ml', quantity: 1, unitPrice: 450, totalPrice: 450, consumedAt: new Date(today.getTime() - 8 * 60 * 60 * 1000), postedToFolio: true, postedAt: new Date(today.getTime() - 8 * 60 * 60 * 1000), consumedBy: 'Guest Rahul Banerjee' },
        { id: uuid('mbcons-4'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), bookingId: uuid('booking-2'), folioId: uuid('folio-2'), roomId: uuid('room-801'), itemId: uuid('mbitem-10'), itemName: 'Champagne Mini 187ml', quantity: 1, unitPrice: 1200, totalPrice: 1200, consumedAt: new Date(today.getTime() - 10 * 60 * 60 * 1000), postedToFolio: true, postedAt: new Date(today.getTime() - 10 * 60 * 60 * 1000), consumedBy: 'Guest Rahul Banerjee', notes: 'Celebration drink' },
        { id: uuid('mbcons-5'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), bookingId: uuid('booking-6'), folioId: uuid('folio-6'), roomId: uuid('room-305'), itemId: uuid('mbitem-1'), itemName: 'Coca-Cola Can', quantity: 1, unitPrice: 80, totalPrice: 80, consumedAt: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000), postedToFolio: true, postedAt: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000), consumedBy: 'Guest Rina Chatterjee' },
      ],
    });
    console.log('10 minibar items, 3 setups, 5 consumption records seeded!');
  } catch (e: any) {
    console.log('Minibar seed error:', e.message);
  }
  ...
  try {
    await prisma.lostFoundItem.createMany({
      data: [
        { id: uuid('lf-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), itemType: 'found', category: 'electronics', description: 'iPhone 15 Pro charger (white, USB-C)', locationFound: 'Room 501 - bedside table', roomId: uuid('room-501'), foundBy: 'Anita Roy (HK Staff)', foundAt: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000), status: 'reported', storageLocation: 'HK Office - Shelf A3', guestId: uuid('guest-1'), bookingId: uuid('booking-1'), notes: 'Left behind after checkout. Guest notified via WhatsApp.' },
        { id: uuid('lf-2'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), itemType: 'found', category: 'accessories', description: 'Gold wrist watch with brown leather strap', locationFound: 'Hotel Restaurant - Table T2', foundBy: 'Suman (Restaurant Staff)', finderContact: '+91-9876543210', foundAt: new Date(today.getTime() - 3 * 24 * 60 * 60 * 1000), status: 'matched', matchedAt: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000), storageLocation: 'Front Desk Safe - Locker 5', returnedTo: 'Vikram Singh', returnedAt: new Date(today.getTime() - 1.5 * 24 * 60 * 60 * 1000), guestId: uuid('guest-5'), bookingId: uuid('booking-4') },
        { id: uuid('lf-3'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), itemType: 'lost', category: 'documents', description: 'Passport - Indian (Name: Sneha Gupta)', locationFound: null, guestId: uuid('guest-2'), bookingId: uuid('booking-3'), status: 'reported', notes: 'Guest reported missing passport. Security team checking CCTV footage.' },
        { id: uuid('lf-4'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), itemType: 'found', category: 'clothing', description: 'Black blazer - size M, Hugo Boss label', locationFound: 'Conference Room B', foundBy: 'Priya Das (FD)', foundAt: new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000), status: 'reported', storageLocation: 'Lost & Found Storage Room - Rack B2' },
        { id: uuid('lf-5'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), itemType: 'found', category: 'electronics', description: 'Samsung Galaxy Buds 2 Pro (white case)', locationFound: 'Gym - locker area', foundBy: 'Raj (Gym Staff)', foundAt: new Date(today.getTime() - 14 * 24 * 60 * 60 * 1000), status: 'disposed', disposedAt: new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000), disposalReason: 'Unclaimed after 7 days. Per policy, disposed.' },
        { id: uuid('lf-6'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), itemType: 'found', category: 'other', description: 'Children\'s storybook - "The Very Hungry Caterpillar"', locationFound: 'Lobby - waiting area sofa', foundBy: 'Anita Roy (HK Staff)', foundAt: new Date(today.getTime() - 5 * 24 * 60 * 60 * 1000), status: 'claimed', matchedAt: new Date(today.getTime() - 4 * 24 * 60 * 60 * 1000), returnedTo: 'Guest family (checked out)', returnedAt: new Date(today.getTime() - 4 * 24 * 60 * 60 * 1000), storageLocation: 'HK Office - Shelf A1' },
        { id: uuid('lf-7'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), itemType: 'found', category: 'accessories', description: 'Ray-Ban sunglasses (black, aviator style)', locationFound: 'Poolside deck chair #7', foundBy: 'Pool attendant', foundAt: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000), status: 'reported', storageLocation: 'Front Desk Safe - Locker 2' },
      ],
    });
    console.log('7 lost & found items seeded!');
  } catch (e: any) {
    console.log('Lost & found seed error:', e.message);
  }

  // ─── A6. Laundry Management ──────────────────────────────────
  console.log('Seeding laundry data...');
  try {
    await prisma.laundryItem.createMany({
      data: [
        ...
        { id: uuid('li-5'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Bed Sheet - Commercial Wash', category: 'linen', serviceType: 'wash', unitPrice: 40, currency: 'INR', turnaroundHours: 8, isActive: true, sortOrder: 5 },
        { id: uuid('li-6'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Bath Towel - Commercial Wash', category: 'linen', serviceType: 'wash', unitPrice: 25, currency: 'INR', turnaroundHours: 8, isActive: true, sortOrder: 6 },
    ...
    await prisma.laundryOrder.createMany({
      data: [
        { id: uuid('lord-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), bookingId: uuid('booking-1'), guestId: uuid('guest-1'), roomId: uuid('room-501'), orderType: 'guest', status: 'delivered', receivedAt: new Date(today.getTime() - 1.5 * 24 * 60 * 60 * 1000), readyAt: new Date(today.getTime() - 1.2 * 24 * 60 * 60 * 1000), deliveredAt: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000), totalItems: 4, totalPrice: 760, currency: 'INR', folioId: uuid('folio-1'), postedToFolio: true, collectedBy: uuid('user-3'), deliveredBy: uuid('user-3'), specialInstructions: 'Express service requested. Gentle cycle for silk items.' },
        { id: uuid('lord-2'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), bookingId: uuid('booking-2'), guestId: uuid('guest-3'), roomId: uuid('room-801'), orderType: 'guest', status: 'in_progress', receivedAt: new Date(today.getTime() - 8 * 60 * 60 * 1000), totalItems: 3, totalPrice: 950, currency: 'INR', folioId: uuid('folio-2'), specialInstructions: 'VIP guest — priority processing. Suit needs stain removal on left lapel.', notes: 'VIP Platinum guest. Coordinated with butler service.' },
    ...
    await prisma.laundryOrderItem.createMany({
      data: [
        ...
        { id: uuid('lori-2-1'), orderId: uuid('lord-2'), itemId: uuid('li-3'), itemName: 'Suit - Dry Clean', serviceType: 'dry_clean', quantity: 1, unitPrice: 500, totalPrice: 500, status: 'in_progress', notes: 'Stain removal on left lapel' },
        ...
        { id: uuid('lori-4-1'), orderId: uuid('lord-4'), itemId: uuid('li-7'), itemName: 'Drape / Curtain - Dry Clean', serviceType: 'dry_clean', quantity: 4, unitPrice: 600, totalPrice: 2400, status: 'received' },
      ],
    });
    console.log('8 laundry items, 6 orders, 8 order items seeded!');
  } catch (e: any) {
    console.log('Laundry seed error:', e.message);
  }

  // ─── A7. Package Plans / Rate Bundling ───────────────────────
  console.log('Seeding package plans data...');
  try {
    await prisma.packagePlan.createMany({
      data: [
        { id: uuid('pkg-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Honeymoon Bliss Package', description: 'Romantic getaway with breakfast, spa, and late checkout for couples', baseRoomTypeId: uuid('roomtype-3'), roomRateInclusive: true, startDate: new Date('2024-01-01'), endDate: new Date('2025-12-31'), minNights: 2, maxNights: 5, totalBasePrice: 25000, currency: 'INR', sortOrder: 1, status: 'active' },
        { id: uuid('pkg-2'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Business Executive Package', description: 'All-inclusive corporate stay with breakfast, airport transfer, and meeting room access', baseRoomTypeId: uuid('roomtype-2'), roomRateInclusive: true, startDate: new Date('2024-01-01'), endDate: new Date('2025-12-31'), minNights: 1, maxNights: 7, totalBasePrice: 12000, currency: 'INR', sortOrder: 2, status: 'active' },
        { id: uuid('pkg-3'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Family Fun Weekend', description: 'Family package with breakfast, dinner, kids activities, and pool access', baseRoomTypeId: uuid('roomtype-2'), roomRateInclusive: false, startDate: new Date('2024-04-01'), endDate: new Date('2025-03-31'), minNights: 2, maxNights: 3, totalBasePrice: 18000, currency: 'INR', sortOrder: 3, status: 'active' },
        { id: uuid('pkg-4'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Royal Experience (Presidential)', description: 'Ultimate luxury: butler service, all meals, spa, limo transfer, private dining', baseRoomTypeId: uuid('roomtype-4'), roomRateInclusive: true, startDate: new Date('2024-01-01'), endDate: new Date('2025-12-31'), minNights: 2, totalBasePrice: 85000, currency: 'INR', sortOrder: 4, status: 'active' },
        { id: uuid('pkg-5'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Staycation Express', description: 'Quick 1-night getaway with breakfast and late checkout', baseRoomTypeId: uuid('roomtype-1'), roomRateInclusive: true, startDate: new Date('2024-06-01'), endDate: new Date('2025-05-31'), minNights: 1, maxNights: 1, totalBasePrice: 5500, currency: 'INR', sortOrder: 5, status: 'active' },
      ],
    });
    await prisma.packageComponent.createMany({
      data: [
        ...
        { id: uuid('pkgcomp-1-4'), packagePlanId: uuid('pkg-1'), componentType: 'other', referenceName: 'Welcome amenities (flowers, chocolates)', includedQty: 1, unitCost: 500, isIncluded: true, sortOrder: 4 },
        ...
        { id: uuid('pkgcomp-2-4'), packagePlanId: uuid('pkg-2'), componentType: 'laundry', referenceName: 'Daily Laundry (3 pieces)', includedQty: 3, unitCost: 200, isIncluded: true, sortOrder: 4 },
        { id: uuid('pkgcomp-3-1'), packagePlanId: uuid('pkg-3'), componentType: 'meal', referenceName: 'Breakfast + Dinner for 2 adults + 1 child', includedQty: 2, unitCost: 2500, isIncluded: true, sortOrder: 1 },
        ...
        { id: uuid('pkgcomp-4-1'), packagePlanId: uuid('pkg-4'), componentType: 'meal', referenceName: 'All Meals (Breakfast, Lunch, Dinner)', includedQty: 3, unitCost: 5000, isIncluded: true, sortOrder: 1 },
        ...
        { id: uuid('pkgcomp-4-3'), packagePlanId: uuid('pkg-4'), componentType: 'airport_transfer', referenceName: 'Luxury Limousine Transfer', includedQty: 2, unitCost: 5000, isIncluded: true, sortOrder: 3 },
      ],
    });
    await prisma.packageRate.createMany({
      data: [
    ...
    console.log('5 package plans, 13 components, 5 rates seeded!');
  } catch (e: any) {
    console.log('Package plans seed error:', e.message);
  }

  // ─── A8. Scheduled / Recurring Charges ───────────────────────
  console.log('Seeding scheduled charges data...');
  try {
    await prisma.scheduledCharge.createMany({
      data: [
        { id: uuid('sc-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), folioId: uuid('folio-1'), bookingId: uuid('booking-1'), chargeType: 'room_charge', description: 'Daily room charge - Deluxe Room 501', category: 'Room', amount: 5500, currency: 'INR', frequency: 'daily', startDate: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000), endDate: new Date(today.getTime() + 1 * 24 * 60 * 60 * 1000), nextExecutionAt: new Date(today.getTime() + 1 * 24 * 60 * 60 * 1000), lastExecutedAt: new Date(), isActive: true, executedCount: 2 },
        { id: uuid('sc-2'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), folioId: uuid('folio-1'), bookingId: uuid('booking-1'), chargeType: 'resort_fee', description: 'Resort fee - pool, gym, WiFi', category: 'Service', amount: 500, currency: 'INR', frequency: 'daily', startDate: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000), endDate: new Date(today.getTime() + 1 * 24 * 60 * 60 * 1000), nextExecutionAt: new Date(today.getTime() + 1 * 24 * 60 * 60 * 1000), lastExecutedAt: new Date(), isActive: true, executedCount: 2 },
        { id: uuid('sc-3'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), folioId: uuid('folio-3'), bookingId: uuid('booking-3'), chargeType: 'room_charge', description: 'Daily room charge - Deluxe Room 510', category: 'Room', amount: 5500, currency: 'INR', frequency: 'daily', startDate: new Date(), endDate: new Date(today.getTime() + 4 * 24 * 60 * 60 * 1000), nextExecutionAt: new Date(today.getTime() + 1 * 24 * 60 * 60 * 1000), isActive: true, executedCount: 0 },
        { id: uuid('sc-4'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), folioId: uuid('folio-2'), bookingId: uuid('booking-2'), chargeType: 'minibar', description: 'Daily minibar restocking fee', category: 'F&B', amount: 200, currency: 'INR', frequency: 'daily', startDate: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000), endDate: new Date(today.getTime() + 3 * 24 * 60 * 60 * 1000), nextExecutionAt: new Date(today.getTime() + 1 * 24 * 60 * 60 * 1000), lastExecutedAt: new Date(), isActive: true, executedCount: 1 },
        { id: uuid('sc-5'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), folioId: uuid('folio-1'), bookingId: uuid('booking-1'), chargeType: 'incidentals', description: 'Extra bed charge', category: 'Room', amount: 1500, currency: 'INR', frequency: 'once', startDate: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000), nextExecutionAt: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000), lastExecutedAt: new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000), isActive: false, executedCount: 1, maxAmount: 1500 },
        { id: uuid('sc-6'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), folioId: uuid('folio-4'), bookingId: uuid('booking-4'), chargeType: 'resort_fee', description: 'Presidential Suite premium amenity fee', category: 'Service', amount: 2000, currency: 'INR', frequency: 'daily', startDate: new Date(), endDate: new Date(today.getTime() + 2 * 24 * 60 * 60 * 1000), nextExecutionAt: new Date(today.getTime() + 1 * 24 * 60 * 60 * 1000), isActive: true, executedCount: 0 },
      ],
    });
    console.log('6 scheduled charges seeded!');
  } catch (e: any) {
    console.log('Scheduled charges seed error:', e.message);
  }

  // ─── A9. Posting Rules / Auto-Charge Routing ─────────────────
  console.log('Seeding posting rules and revenue accounts data...');
  try {
    await prisma.revenueAccount.createMany({
      data: [
        { id: uuid('ra-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), code: 'ROOM-REVENUE', name: 'Room Revenue', accountType: 'revenue', category: 'room', description: 'All room charge revenue', isActive: true, sortOrder: 1 },
        { id: uuid('ra-2'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), code: 'FB-REVENUE', name: 'Food & Beverage Revenue', accountType: 'revenue', category: 'food_beverage', description: 'Restaurant, room service, minibar F&B', isActive: true, sortOrder: 2 },
        { id: uuid('ra-3'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), code: 'MINIBAR-REVENUE', name: 'Minibar Revenue', accountType: 'revenue', category: 'minibar', description: 'Minibar consumption charges', isActive: true, sortOrder: 3 },
        { id: uuid('ra-4'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), code: 'LAUNDRY-REVENUE', name: 'Laundry Revenue', accountType: 'revenue', category: 'laundry', description: 'Guest and housekeeping laundry charges', isActive: true, sortOrder: 4 },
        { id: uuid('ra-5'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), code: 'SPA-REVENUE', name: 'Spa Revenue', accountType: 'revenue', category: 'other', description: 'Spa and wellness services', isActive: true, sortOrder: 5 },
        { id: uuid('ra-6'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), code: 'PARKING-REVENUE', name: 'Parking Revenue', accountType: 'revenue', category: 'other', description: 'Valet and self-parking charges', isActive: true, sortOrder: 6 },
        { id: uuid('ra-7'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), code: 'SERVICE-CHARGE', name: 'Service Charges', accountType: 'revenue', category: 'miscellaneous', description: 'Resort fees, service charges, surcharges', isActive: true, sortOrder: 7 },
        { id: uuid('ra-8'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), code: 'TAX-COLLECTED', name: 'Tax Collected (GST)', accountType: 'liability', category: 'miscellaneous', description: 'GST collected from guests', isActive: true, sortOrder: 8 },
      ],
    });
    await prisma.postingRule.createMany({
      data: [
        { id: uuid('prule-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Auto-Post Room Charges', description: 'Automatically post daily room charges to guest folio at midnight', chargeCategory: 'Room', chargeType: 'room_charge', revenueAccountId: uuid('ra-1'), taxTreatment: 'taxable', autoPost: true, isActive: true, priority: 10, conditions: JSON.stringify({ trigger: 'night_audit', frequency: 'daily', postTo: 'guest_folio' }) },
        { id: uuid('prule-2'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Auto-Post Minibar Charges', description: 'Post minibar consumption charges to guest folio on checkout', chargeCategory: 'F&B', chargeType: 'minibar', revenueAccountId: uuid('ra-3'), taxTreatment: 'taxable', autoPost: true, isActive: true, priority: 5, conditions: JSON.stringify({ trigger: 'checkout', postTo: 'guest_folio', includeTax: true }) },
        { id: uuid('prule-3'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Auto-Post Laundry Charges', description: 'Post completed laundry charges to guest folio', chargeCategory: 'Service', chargeType: 'laundry', revenueAccountId: uuid('ra-4'), taxTreatment: 'taxable', autoPost: true, isActive: true, priority: 5, conditions: JSON.stringify({ trigger: 'laundry_delivered', postTo: 'guest_folio' }) },
        { id: uuid('prule-4'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Resort Fee Auto-Post', description: 'Post daily resort fee for amenity access', chargeCategory: 'Service', chargeType: 'resort_fee', revenueAccountId: uuid('ra-7'), taxTreatment: 'taxable', autoPost: true, isActive: true, priority: 8, conditions: JSON.stringify({ trigger: 'night_audit', frequency: 'daily', exemptRoomTypes: ['roomtype-1'], postTo: 'guest_folio' }) },
        { id: uuid('prule-5'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'No-Show Penalty Charge', description: 'Charge no-show guests as per cancellation policy', chargeCategory: 'Room', chargeType: 'penalty', revenueAccountId: uuid('ra-1'), taxTreatment: 'taxable', autoPost: true, isActive: true, priority: 9, conditions: JSON.stringify({ trigger: 'no_show', chargePercent: 100, applyTo: 'booking_total' }) },
        { id: uuid('prule-6'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Extended Stay Surcharge', description: '10% surcharge for stays beyond planned checkout', chargeCategory: 'Room', chargeType: 'extension', revenueAccountId: uuid('ra-1'), taxTreatment: 'taxable', autoPost: true, isActive: true, priority: 7, conditions: JSON.stringify({ trigger: 'extension', surchargePercent: 10 }) },
      ],
    });
    await prisma.postingLog.createMany({
      data: [
        { id: uuid('plog-1'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), ruleId: uuid('prule-1'), folioId: uuid('folio-1'), chargeAmount: 5500, revenueAccountCode: 'ROOM-REVENUE', postedAt: new Date(), postedBy: 'system', autoPosted: true },
        { id: uuid('plog-2'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), ruleId: uuid('prule-1'), folioId: uuid('folio-2'), chargeAmount: 12000, revenueAccountCode: 'ROOM-REVENUE', postedAt: new Date(), postedBy: 'system', autoPosted: true },
        { id: uuid('plog-3'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), ruleId: uuid('prule-2'), folioId: uuid('folio-1'), chargeAmount: 360, revenueAccountCode: 'MINIBAR-REVENUE', postedAt: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000), postedBy: 'system', autoPosted: true },
        { id: uuid('plog-4'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), ruleId: uuid('prule-4'), folioId: uuid('folio-1'), chargeAmount: 500, revenueAccountCode: 'SERVICE-CHARGE', postedAt: new Date(), postedBy: 'system', autoPosted: true },
        { id: uuid('plog-5'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), ruleId: uuid('prule-3'), folioId: uuid('folio-2'), chargeAmount: 760, revenueAccountCode: 'LAUNDRY-REVENUE', postedAt: new Date(today.getTime() - 1 * 24 * 60 * 60 * 1000), postedBy: 'system', autoPosted: true },
      ],
    });
