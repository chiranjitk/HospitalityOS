  ...
  try {
    await prisma.pricingRule.createMany({
      data: [
        ...
        // ── Stay-Based ────────────────────────────────────────────
        // pr-12: long_stay
        { id: uuid('pr-12'), tenantId: uuid('tenant-1'), propertyId: uuid('property-1'), name: 'Extended Stay Discount', type: 'long_stay', description: '12% discount for stays of 7+ nights', value: 12, valueType: 'percentage', conditions: JSON.stringify({ minStay: 7 }), priority: 3, isActive: true, effectiveFrom: new Date(today.getTime() - 120 * 24 * 60 * 60 * 1000), roomTypes: JSON.stringify(['roomtype-1', 'roomtype-2']), appliedCount: 29, lastAppliedAt: new Date(today.getTime() - 4 * 24 * 60 * 60 * 1000) },
    ...
    console.log('19 pricing rules seeded (all 15 types + 1 legacy + 3 property-2)!');
  } catch (e: any) {
