...
success "PM2 configured — 7 services started"
...
if [[ -f "${APP_DIR}/freeradius-config-patches/setup-production.sh" ]]; then
  ...
  bash "${APP_DIR}/freeradius-config-patches/setup-production.sh" 2>&1 | tail -10 || warn "Production patches failed (non-fatal)"
fi
...
if radiusd -XC >/dev/null 2>&1; then
  ...
  warn "FreeRADIUS config check failed — skipping restart (check manually)"
fi
...
(crontab -l 2>/dev/null | grep -v "fr_new_data_usage_period\|fr_update_data_usage\|staysuite-cleanup"; echo "
# StaySuite: Hourly data usage period creation
0 * * * * sudo -u postgres psql -d staysuite -c 'SELECT fr_new_data_usage_period();' > /dev/null 2>&1

# StaySuite: Data usage aggregation every 5 minutes
*/5 * * * * sudo -u postgres psql -d staysuite -c 'SELECT fr_update_data_usage();' > /dev/null 2>&1

# StaySuite: Cleanup old data_usage_by_period (>90 days) at 3 AM
...
for svc_name in staysuite-nextjs availability-service realtime-service freeradius-service dhcp-service dns-service nftables-service; do
