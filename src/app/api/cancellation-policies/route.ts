  ...
  try {
    ...
    const policy = await db.cancellationPolicy.create({
      data: {
        ...
        freeCancelHoursBefore: freeCancelHoursBefore !== undefined ? Math.max(0, Number(freeCancelHoursBefore)) : 48,
        penaltyPercent: penaltyPercent !== undefined ? Math.min(100, Math.max(0, Number(penaltyPercent))) : 50,
        noShowPenaltyPercent: noShowPenaltyPercent !== undefined ? Math.min(100, Math.max(0, Number(noShowPenaltyPercent))) : 100,
        penaltyType: penaltyType || 'percentage',
        penaltyFixedAmount: penaltyFixedAmount !== undefined ? Number(Number(penaltyFixedAmount).toFixed(2)) : null,
        penaltyNights: penaltyNights !== undefined ? Math.max(1, Math.floor(Number(penaltyNights))) : null,
