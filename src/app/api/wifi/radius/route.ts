    ...
    switch (action) {
        ...
        try {
          ...
          if (newStatus === 'suspended' || newStatus === 'deactivated') {
            try {
                ...
                for (const session of activeSessions) {
                  ...
                  try {
                    ...
                    try {
                      ...
                      try { fs.unlinkSync(tmpAttrsFile); } catch (unlinkErr) { /* ignore */ }
                    }
                  } catch (coaErr) {
                    console.warn('[change-user-status] CoA disconnect failed for ' + user.username + ':', coaErr);
                  }

                  // Always close the session in database
                  try {
                    const closeSql = [
                      'UPDATE radacct',
              ...
              console.warn('[change-user-status] Session disconnect error:', sessionErr);
            }
          }
      ...
      case 'fap-policies-create': {
        try {
          ...
          const { name, description, cycleType, limitType, dataLimitMb, dataLimitUnit, applicableOn,
          ...
          `, id, tenantId, propertyId, name, description || null, cycleType || 'daily', limitType || 'total',
