...
import { Briefcase, TreePalm, Users, Crown, CalendarDays, RefreshCw, AlertCircle } from 'lucide-react';
...
function DonutChart({ segments }: { segments: GuestSegment[] }) {
  ...
  const radius = 52; const circumference = 2 * Math.PI * radius; const gap = 2;
  const segmentArcs = segments.reduce<Array<{ segment: GuestSegment; segmentLength: number; offset: number }>>((acc, segment) => {
    ...
    <div className="relative flex items-center justify-center">
      <svg width="128" height="128" viewBox="0 0 128 128" className="transform -rotate-90">
        {segmentArcs.map(({ segment, segmentLength, offset }) => (
          <circle key={segment.id} cx="64" cy="64" r={radius} fill="none" stroke={segment.color} strokeWidth="16" strokeDasharray={`${Math.max(0, segmentLength - gap)} ${circumference - segmentLength + gap}`} strokeDashoffset={-offset} strokeLinecap="round" className="transition-all duration-700 ease-out" />
        ))}
      </svg>
      <div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-xl font-bold tabular-nums">{total}</span><span className="text-[10px] text-muted-foreground leading-none">{total}</span></div>
    </div>
  );
}
...
function SegmentCard({ segment, index }: { segment: GuestSegment; index: number }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.07, duration: 0.3 }} className={cn('flex items-center gap-3 p-2.5 rounded-xl transition-all duration-200', 'hover:bg-muted/50 cursor-default')}>
      <div className="flex-shrink-0 h-8 w-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${segment.color}15`, color: segment.color }}><Icon className="h-4 w-4" /></div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between"><span className="text-sm font-medium truncate">{segment.name}</span><span className="text-xs font-semibold tabular-nums" style={{ color: segment.color }}>{segment.percentage}%</span></div>
        <div className="flex items-center justify-between mt-0.5"><span className="text-xs text-muted-foreground">{segment.count} guests</span><div className="h-1.5 w-16 rounded-full bg-muted overflow-hidden"><motion.div className="h-full rounded-full" style={{ backgroundColor: segment.color }} initial={{ width: 0 }} animate={{ width: `${segment.percentage}%` }} transition={{ delay: index * 0.1 + 0.3, duration: 0.6, ease: 'easeOut' }} /></div></div>
      </div>
    </motion.div>
  );
}
...
export function GuestSegmentsWidget() {
  ...
  useEffect(() => { fetchData(true); const interval = setInterval(() => fetchData(false), 5 * 60 * 1000); return () => clearInterval(interval); }, [fetchData]);
    ...
    <Card className="border border-border/50 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 rounded-2xl">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base font-semibold flex items-center gap-2"><Users className="h-4 w-4 text-violet-600 dark:text-violet-400" />{t('guestSegments')}</CardTitle>
