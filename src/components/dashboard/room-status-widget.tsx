...
function OccupancyRing({ value, size = 100 }: { value: number; size?: number }) {
  ...
  const ringColor = value >= 90 ? 'text-red-500 dark:text-red-400' : value >= 70 ? 'text-amber-500 dark:text-amber-400' : value >= 40 ? 'text-primary dark:text-primary' : 'text-teal-500 dark:text-teal-400';
...
function DistributionBar({ statusCounts, totalRooms, t }: { statusCounts: RoomStatusCounts; totalRooms: number; t: (key: string) => string }) {
  const statusItems: { key: keyof RoomStatusCounts; labelKey: string; bar: string; tooltipKey: string }[] = [
    { key: 'available', labelKey: 'available', bar: 'bg-primary', tooltipKey: 'availableTooltip' },
            ...
            <div className="flex-1 space-y-2">
              <div className="flex flex-wrap gap-1.5">
                {['available', 'occupied', 'dirty', 'maintenance', 'out_of_order'].map((key) => {
                  ...
                  const colors: Record<string, string> = { available: 'text-primary', occupied: 'text-violet-500', dirty: 'text-orange-500', maintenance: 'text-amber-500', out_of_order: 'text-slate-500' };
                  ...
                  return (<Tooltip key={key}><TooltipTrigger asChild><div className={cn('inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium cursor-default transition-transform hover:scale-105', key === 'available' ? 'bg-primary/10' : key === 'dirty' ? 'bg-orange-500/10' : key === 'maintenance' ? 'bg-amber-500/10' : 'bg-slate-500/10', `text-${colors[key]}`)}>
              ...
              <div className="flex items-center gap-3 text-xs text-muted-foreground">
                ...
                <span className="flex items-center gap-1"><span className="inline-block h-1.5 w-1.5 rounded-full bg-primary" /><span className="tabular-nums">{data.statusCounts.available}</span> {t('available')}</span>
              </div>
            </div>
          </div>
