...
type RuleType = 'markup' | 'markdown' | 'discount_percentage' | 'discount_fixed' | 'surcharge_percentage' | 'surcharge_fixed' | 'early_bird' | 'last_minute' | 'long_stay' | 'weekend' | 'seasonal' | 'promo_code' | 'occupancy' | 'advance_booking' | 'channel';
...
export default function RatePlansPricingRules() {
  ...
  const getPriceForDay = (day: Date, roomTypeId: string): number => {
    ...
    const weekendDay = isWeekend(day); // Sat or Sun
    
    // Sort rules by priority (higher priority = applied first)
    const applicableRules = pricingRules
      .filter(r => r.isActive && (!r.roomTypes.length || r.roomTypes.includes(roomTypeId)))
    ...
    for (const rule of applicableRules) {
      ...
      // Check if rule applies to this day via daysOfWeek condition
      if (rule.conditions?.daysOfWeek?.length && !rule.conditions.daysOfWeek.includes(dayOfWeek)) continue;

      // Apply adjustment
      if (rule.valueType === 'percentage') {
        const multiplier = isDiscountType ? -rule.value : rule.value;
        price = price * (1 + multiplier / 100);
      } else {
        const amount = isDiscountType ? -rule.value : rule.value;
        price = price + amount;
      }
    }
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <div className="mt-4 p-4 bg-muted/30 rounded-lg">
                ...
                <div className="flex flex-wrap gap-2">
                  {pricingRules.filter(r => r.isActive).slice(0, 5).map(rule => {
                    const isDiscount = DISCOUNT_TYPES.includes(rule.type as RuleType);
                    return (
                      <Badge key={rule.id} variant="outline" className={cn("gap-1", ruleTypeColors[rule.type] || 'bg-gray-100 text-gray-700')}>
                        {isDiscount ? <TrendingDown className="h-3 w-3" /> : <TrendingUp className="h-3 w-3" />}
                        {rule.name}: {isDiscount ? '-' : '+'}{rule.value}{rule.valueType === 'percentage' ? '%' : ''}
                      </Badge>
                    );
                  })}
        ...
        <TabsContent value="rules" className="mt-6 space-y-4">
          ...
          {/* Rules list with Create button and full table */}
          <div className="flex justify-between items-center">
            <CardDescription>Dynamic adjustments (weekday, demand, events, promo codes)</CardDescription>
                      ...
                      {pricingRules.map((rule) => {
                        ...
                        return (
                          <TableRow key={rule.id} className={!rule.isActive ? 'opacity-60' : ''}>
                            ...
                            <TableCell>
                              <Badge className={ruleTypeColors[rule.type] || 'bg-gray-100 text-gray-700'}>
                            ...
                            <TableCell>
                              <div className="flex items-center gap-1">
                                ...
                                <span className={isDiscount ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'}>
                                  {isDiscount ? '-' : '+'}{rule.value}{rule.valueType === 'percentage' ? '%' : ''}
                                </span>
                              </div>
                            </TableCell>
                            <TableCell>
...
// Rule Form Component — Full version with all 15 rule types
function RuleForm({ rule, onChange, onSave, onCancel, isSaving, currencySymbol }: {
  ...
  return (
    <div className="space-y-4 py-4">
      ...
      {/* Weekend */}
      {currentType === 'weekend' && (
        <div className="space-y-2 rounded-md border border-violet-200 bg-violet-50/50 dark:border-violet-800 dark:bg-violet-950/30 p-3">
          <label className="text-xs font-medium flex items-center gap-1.5 text-violet-700 dark:text-violet-400">
            <Calendar className="h-3.5 w-3.5" />
            Weekend Adjustment
          </label>
      ...
      {!['early_bird', 'last_minute', 'long_stay', 'promo_code', 'occupancy', 'advance_booking', 'channel', 'weekend'].includes(currentType) && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2">
