...
// Deep set a value in nested object using dot-notation key
...
async function main() {
  // Load locale file - keep as nested object, work with flat keys
  ...
  console.error(`${LANG}: ${remaining.length} remaining to translate`);
  ...
  for (let i = 0; i < remaining.length; i += BATCH) {
    ...
    // Save to done file AND apply directly
  ...
  fs.writeFileSync(`src/messages/${LANG}.json`, JSON.stringify(locale, null, 2) + '\n', 'utf-8');
  console.error(`${LANG}: DONE - ${total} translated`);
}
