...
if [[ "$DN_CLASSID" -gt 0 ]]; then
    # Delete flower filter matching this IP's fwmark
    tc filter del dev ifb0 parent 1: protocol ip pref "$FW_PREF" flower \
        fwmark "${MARK}/0xFFFFFFFF" 2>/dev/null && log_msg "tc: del flower filter ifb0 $MARK"

    # Fallback: delete by scanning pref for this fwmark
    tc filter show dev ifb0 parent 1: 2>/dev/null | grep -q "fwmark 0x" && {
        fh=$(tc filter show dev ifb0 parent 1: 2>/dev/null \
            | grep -B3 "${MARK#0x}" | grep -oP 'pref \K[0-9]+' | head -1) || true
        [[ -n "$fh" ]] && tc filter del dev ifb0 parent 1: protocol ip pref "$fh" 2>/dev/null \
            && log_msg "tc: del flower filter ifb0 by pref $fh"
    }
...
if [[ "$UP_CLASSID" -gt 0 ]]; then
    tc filter del dev ifb1 parent 1: protocol ip pref "$FW_PREF" flower \
        fwmark "${MARK}/0xFFFFFFFF" 2>/dev/null && log_msg "tc: del flower filter ifb1 $MARK"

    tc filter show dev ifb1 parent 1: 2>/dev/null | grep -q "fwmark 0x" && {
        fh=$(tc filter show dev ifb1 parent 1: 2>/dev/null \
            | grep -B3 "${MARK#0x}" | grep -oP 'pref \K[0-9]+' | head -1) || true
        [[ -n "$fh" ]] && tc filter del dev ifb1 parent 1: protocol ip pref "$fh" 2>/dev/null \
            && log_msg "tc: del flower filter ifb1 by pref $fh"
    }
